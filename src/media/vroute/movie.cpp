...
int movie_interrupt(void *ctx) {
  MovieCapture *movie = (MovieCapture *)ctx;
...
void MovieCapture::free_pkt() {
  av_packet_free(&pkt);
  pkt = nullptr;
}

void MovieCapture::free_nal() {
  if (nal_info) {
    free(nal_info);
    nal_info = nullptr;
    nal_info_len = 0;
...
int MovieCapture::timelapse_exists(const char *fname) {
  struct stat statbuf;
...
int MovieCapture::timelapse_append(AVPacket *p_pkt) {
  FILE *file;
...
void MovieCapture::free_context() {
  if (picture != nullptr) {
...
int MovieCapture::set_pts(const struct timespec *ts1) {
  ...
  if (tlapse != TIMELAPSE_NONE) {
    ...
    pts_interval = ((1000000L * (ts1->tv_sec - start_time.tv_sec)) +
                    (ts1->tv_nsec / 1000) - (start_time.tv_nsec / 1000));
    if (pts_interval < 0) {
      /* This can occur when we have pre-capture frames.  Reset start time of
       * video. */
    ...
    if (last_pts < 0) {
      ...
      picture->pts =
          base_pts + av_rescale_q(pts_interval, av_make_q(1, 1000000L),
                                  strm_video->time_base);
    }
    if (test_mode == true) {
      MOTPLS_LOG(INF, TYPE_ENCODER, NO_ERRNO,
                 _("PTS %" PRId64 " Base PTS %" PRId64 " ms interval %" PRId64
                   " timebase %d-%d"),
    ...
    if (picture->pts <= last_pts) {
      // We have a problem with our motion loop timing and sending frames or the
      // rounding into the PTS.
      if (test_mode == true) {
        MOTPLS_LOG(INF, TYPE_ENCODER, NO_ERRNO,
                   _("BAD TIMING!! Frame skipped."));
      }
      return -1;
    }
...
int MovieCapture::set_quality() {
  ...
  if (ctx_codec->codec_id == AV_CODEC_ID_H264 ||
    ...
    /* The selection of 8000 is a subjective number based upon viewing output
     * files */
    if (quality > 0) {
      quality =
          (int)(((100 - quality) * (100 - quality) * (100 - quality) * 8000) /
                1000000) +
          1;
  ...
  MOTPLS_LOG(INF, TYPE_ENCODER, NO_ERRNO, _("%s codec vbr/crf/bit_rate: %d"),
             codec->name, quality);

  return 0;
}

int MovieCapture::set_codec_preferred() {
  codec = nullptr;
  if (preferred_codec != "") {
    ...
    if (codec == nullptr) {
      ...
      MOTPLS_LOG(NTC, TYPE_ENCODER, NO_ERRNO, _("Using codec %s"),
                 preferred_codec.c_str());
    }
  } else {
    codec = avcodec_find_encoder(oc->video_codec_id);
...
int MovieCapture::set_codec() {
  ...
  if (ctx_codec == nullptr) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               _("Failed to allocate codec context!"));
    free_context();
    return -1;
  }
  ...
  */
  if ((tlapse == TIMELAPSE_NONE) && (fps <= 5)) {
    if ((container == "flv") || (container == "mp4") || (container == "hevc")) {
      MOTPLS_LOG(NTC, TYPE_ENCODER, NO_ERRNO,
                 "Low fps. Encoding %d frames into a %d frames container.", fps,
                 10);
      fps = 10;
    }
  }
  ...
  if (retcd < 0) {
    ...
    if (retcd < 0) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO, _("Could not open codec %s"),
                 errstr);
      av_dict_free(&opts);
      free_context();
      return -1;
...
int MovieCapture::alloc_video_buffer(AVFrame *frame, int align) {
  ...
  if ((ret = av_image_check_size((uint)frame->width, (uint)frame->height, 0,
                                 nullptr)) < 0) {
    return ret;
  }
  ...
  if (!frame->linesize[0]) {
    ...
    for (i = 1; i <= align; i += i) {
      ret = av_image_fill_linesizes(frame->linesize,
  ...
  ret = av_image_fill_pointers(frame->data, (enum AVPixelFormat)frame->format,
                               padded_height, nullptr, frame->linesize);
  if (ret < 0) {
    return ret;
  }
...
int MovieCapture::set_picture() {
  ...
  if (cam->cfg->movie_quality) {
    picture->quality =
        (int)(FF_LAMBDA_MAX * (float)((100 - cam->cfg->movie_quality) / 100)) +
        1;
  }
  ...
  if (preferred_codec == "h264_v4l2m2m") {
    ...
    if (retcd) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO, _("could not alloc buffers %s"),
                 errstr);
      free_context();
      return -1;
    }
...
int MovieCapture::set_outputfile() {
  int retcd;
  char errstr[128];

  /* Open the output file, if needed. */
  if ((timelapse_exists(full_nm.c_str()) == 0) ||
      ...
      if (errno == ENOENT) {
        ...
        if (retcd < 0) {
          ...
          MOTPLS_LOG(ERR, TYPE_ENCODER, SHOW_ERRNO,
                     _("error %s opening file %s"), errstr, full_nm.c_str());
          remove(full_nm.c_str());
          free_context();
          return -1;
        }
        ...
        av_strerror(retcd, errstr, sizeof(errstr));
        MOTPLS_LOG(ERR, TYPE_ENCODER, SHOW_ERRNO, _("avio_open: %s File %s"),
    ...
    while (recv_cd != AVERROR_EOF) {
      ...
      if (recv_cd != AVERROR_EOF) {
        if (recv_cd < 0) {
          ...
          MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO, _("Error draining codec:%s"),
                     errstr);
          free_pkt();
          return -1;
        }
...
int MovieCapture::put_frame(const struct timespec *ts1) {
  int retcd;
  ...
  if (retcd != 0) {
    if (retcd != -2) {
      MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
                 _("Error while encoding picture"));
    }
    free_pkt();
    return retcd;
  }
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               _("Error while writing video frame"));
    return -1;
  }
  return retcd;
}
...
void MovieCapture::passthru_reset() {
  int indx;
...
int MovieCapture::passthru_pktpts() {
  ...
  if (pkt->pts != AV_NOPTS_VALUE) {
    ...
    pkt->pts = av_rescale_q(
        ts_interval, netcam_data->transfer_format->streams[indx]->time_base,
        tmpbase);
  } else {
  ...
  if ((pkt->pts == AV_NOPTS_VALUE) || (pkt->dts == AV_NOPTS_VALUE)) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               "pkt->dts or pkt->pts is AV_NOPTS_VALUE");
    return -1;
  }
...
void MovieCapture::passthru_write(int indx) {
...
void MovieCapture::passthru_minpts() {
...
int MovieCapture::passthru_put(ctx_image_data *img_data) {
...
int MovieCapture::passthru_streams_video(AVStream *stream_in) {
  int retcd;
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               _("Unable to copy video codec parameters"));
    return -1;
  }
  ...
  MOTPLS_LOG(DBG, TYPE_ENCODER, NO_ERRNO, _("video timebase %d/%d fps %d/%d"),
             strm_video->time_base.num, strm_video->time_base.den,
             strm_video->avg_frame_rate.num, strm_video->avg_frame_rate.den);

  return 0;
}

int MovieCapture::passthru_streams_audio(AVStream *stream_in) {
  int retcd;
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               _("Unable to copy audio codec parameters"));
    return -1;
  }
  ...
  MOTPLS_LOG(DBG, TYPE_ENCODER, NO_ERRNO, _("audio timebase %d/%d"),
             strm_audio->time_base.num, strm_audio->time_base.den);
  return 0;
}

int MovieCapture::passthru_streams() {
  int retcd, indx;
  AVStream *stream_in;
...
int MovieCapture::passthru_check() {
  if ((netcam_data->status == NETCAM_NOTCONNECTED) ||
      (netcam_data->status == NETCAM_RECONNECTING)) {
    MOTPLS_LOG(NTC, TYPE_ENCODER, NO_ERRNO,
...
int MovieCapture::passthru_open() {
  int retcd;
  ...
  if (!oc) {
    MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
               _("Could not allocate output context"));
    free_context();
    return -1;
  }
  ...
  if ((container != "mp4") && (container != "mov") && (container != "mkv")) {
    MOTPLS_LOG(NTC, TYPE_ENCODER, NO_ERRNO,
...
void MovieCapture::put_pix_yuv420(ctx_image_data *img_data) {
  unsigned char *image;
  ...
  picture->data[2] =
      picture->data[1] + ((ctx_codec->width * ctx_codec->height) / 4);
}

void MovieCapture::on_movie_start() {
  MOTPLS_LOG(DBG, TYPE_EVENTS, NO_ERRNO, _("Creating movie: %s"),
             full_nm.c_str());
...
void MovieCapture::on_movie_end() {
  MOTPLS_LOG(DBG, TYPE_EVENTS, NO_ERRNO, _("Finished movie: %s"),
             full_nm.c_str());
...
int MovieCapture::movie_open() {
  if (passthrough) {
    if (passthru_open() < 0) {
      MOTPLS_LOG(ERR, TYPE_ENCODER, NO_ERRNO,
                 _("Could not setup passthrough!"));
      free_context();
      return -1;
    }
    return 0;
...
void MovieCapture::stop() {
  ...
  if ((movie_type == "norm") || (movie_type == "motion") ||
      (movie_type == "extpipe")) {
    on_movie_end();
    cam->app->dbse->exec(cam, full_nm, "movie_end");
    if ((cam->cfg->movie_retain == "secondary") &&
        (cam->algsec->detected == false) && (cam->algsec->method != "none")) {
      if (remove(full_nm.c_str()) != 0) {
        MOTPLS_LOG(ERR, TYPE_EVENTS, SHOW_ERRNO, _("Unable to remove file %s"),
                   full_nm.c_str());
      } else {
        cam->app->dbse->filelist_add(cam, ts, "movie", file_nm, full_nm,
                                     file_dir);
      }
    } else {
      cam->app->dbse->filelist_add(cam, ts, "movie", file_nm, full_nm,
...
int MovieCapture::extpipe_put() {
  int retcd;

  retcd = 0;
  if (fileno(extpipe_stream) > 0) {
    if ((cam->imgs.size_high > 0) && (cam->movie_passthrough == false)) {
      if (!fwrite(cam->current_image->image_high, (uint)cam->imgs.size_high, 1,
                  extpipe_stream)) {
      ...
      if (!fwrite(cam->current_image->image_norm, (uint)cam->imgs.size_norm, 1,
                  extpipe_stream)) {
        MOTPLS_LOG(ERR, TYPE_EVENTS, SHOW_ERRNO,
                   _("Error writing in pipe , state error %d"),
                   ferror(extpipe_stream));
...
int MovieCapture::put_image(ctx_image_data *img_data,
...
void MovieCapture::reset_start_time(const struct timespec *ts1) {
  int64_t one_frame_interval =
      av_rescale_q(1, av_make_q(1, fps), strm_video->time_base);
...
void MovieCapture::init_container() {
  int codenbr;
  size_t col_pos;

  if (cam->cfg->movie_container == "test") {
    MOTPLS_LOG(NTC, TYPE_ENCODER, NO_ERRNO,
               "Running test of the various output formats.");
...
void MovieCapture::start_norm() {
  ...
  if (movie_open() < 0) {
    MOTPLS_LOG(ERR, TYPE_EVENTS, NO_ERRNO, _("Error initializing movie."));
    return;
  }
...
void MovieCapture::start_motion() {
  char tmp[PATH_MAX];
...
void MovieCapture::start_timelapse() {
  char tmp[PATH_MAX];

  mystrftime(cam, tmp, sizeof(tmp), cam->cfg->timelapse_filename.c_str(),
             nullptr);
  ...
  if (cam->cfg->timelapse_container == "mpg") {
    ...
    MOTPLS_LOG(NTC, TYPE_EVENTS, NO_ERRNO,
               _("Events will be appended to file"));
    tlapse = TIMELAPSE_APPEND;
    container = "mpg";
  } else {
    MOTPLS_LOG(NTC, TYPE_EVENTS, NO_ERRNO, _("Timelapse using mkv container."));
    MOTPLS_LOG(NTC, TYPE_EVENTS, NO_ERRNO,
               _("Events will be trigger new files"));
...
void MovieCapture::start_extpipe() {
  char tmp[PATH_MAX];
  ...
  mystrftime(cam, tmp, sizeof(tmp), cam->cfg->movie_extpipe.c_str(),
             full_nm.c_str());
...
MovieCapture::MovieCapture(Camera_t *p_cam, std::string pmovie_type) {
  cam = p_cam;
