...
void ImageCapture::picname(char *fullname, std::string fmtstr,
                           std::string basename, std::string extname) {
  char filename[PATH_MAX];
  int retcd;

  mystrftime(cam, filename, sizeof(filename), basename.c_str(), NULL);
...
void ImageCapture::on_picture_save_command(char *fname) {
...
void ImageCapture::process_norm() {
  char filename[PATH_MAX];

  if (cam->cfg->picture_output == "on") {
    picname(filename, "%s/%s.%s", cam->cfg->picture_filename,
    ...
    cam->app->dbse->filelist_add(cam, &cam->current_image->imgts, "pic",
                                 file_nm, full_nm, file_dir);
  }
}

void ImageCapture::process_motion() {
  char filename[PATH_MAX];

  if (cam->cfg->picture_output_motion == "on") {
    picname(filename, "%s/%sm.%s", cam->cfg->picture_filename,
            cam->cfg->picture_type);
    save_norm(filename, cam->imgs.image_motion.image_norm);
    on_picture_save_command(filename);
    cam->app->dbse->exec(cam, filename, "pic_save");
    cam->app->dbse->filelist_add(cam, &cam->imgs.image_motion.imgts, "pic",
                                 file_nm, full_nm, file_dir);

  } else if (cam->cfg->picture_output_motion == "roi") {
    picname(filename, "%s/%sr.%s", cam->cfg->picture_filename,
            cam->cfg->picture_type);
...
void ImageCapture::process_snapshot() {
  char filename[PATH_MAX];
  ...
  if (cam->cfg->snapshot_filename.compare((uint)offset, 8, "lastsnap") != 0) {
    picname(filename, "%s/%s.%s", cam->cfg->snapshot_filename,
            cam->cfg->picture_type);
    if ((cam->imgs.size_high > 0) && (cam->movie_passthrough == false)) {
      save_norm(filename, cam->current_image->image_high);
    } else {
    ...
    picname(linkpath, "%s/%s.%s", "lastsnap", cam->cfg->picture_type);
    remove(linkpath);
...
void ImageCapture::process_preview() {
  char filename[PATH_MAX];
  ...
  if (cam->imgs.image_preview.diffs) {
    ...
    on_picture_save_command(filename);
    cam->app->dbse->exec(cam, filename, "pic_save");
    cam->app->dbse->filelist_add(cam, &cam->imgs.image_preview.imgts, "pic",
                                 file_nm, full_nm, file_dir);
...
#ifdef HAVE_WEBP
void ImageCapture::webp_exif(WebPMux *webp_mux, timespec *ts1, ctx_coord *box) {
  u_char *exif = NULL;
  uint exif_len = jpgutl_exif(&exif, cam, ts1, box);

  if (exif_len > 0) {
    WebPData webp_exif;
    /* EXIF in WEBP does not need the EXIF marker signature (6 bytes) that are
     * needed by jpeg */
...
void ImageCapture::save_webp(FILE *fp, u_char *image, int width, int height,
  ...
  if (!WebPPictureAlloc(&webp_image)) {
    MOTPLS_LOG(ERR, TYPE_CORE, NO_ERRNO,
               _("libwebp image buffer allocation error"));
    return;
  }
  ...
  if (fwrite(webp_output.bytes, sizeof(uint8_t), webp_output.size, fp) !=
      webp_output.size) {
    MOTPLS_LOG(ERR, TYPE_CORE, NO_ERRNO,
               _("unable to save webp image to file"));
  }
...
void ImageCapture::save_yuv420p(FILE *fp, u_char *image, int width, int height,
                                timespec *ts1, ctx_coord *box) {
...
void ImageCapture::save_grey(FILE *picture, u_char *image, int width,
                             int height, timespec *ts1, ctx_coord *box) {
  int sz, image_size;
...
void ImageCapture::save_ppm(FILE *picture, u_char *image, int width,
                            int height) {
  int x, y;
  u_char *l = image;
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      g = 76283 * (((int)*l) - 16) - 53281 * (((int)*u) - 128) -
          25625 * (((int)*v) - 128);
...
int ImageCapture::put_memory(u_char *img_dst, int image_size, u_char *image,
  ...
  if (cam->cfg->stream_grey) {
    retcd = jpgutl_put_grey(img_dst, image_size, image, width, height, quality,
                            cam, &ts1, NULL);
  } else {
    retcd = jpgutl_put_yuv420p(img_dst, image_size, image, width, height,
                               quality, cam, &ts1, NULL);
  }
...
void ImageCapture::pic_write(FILE *picture, u_char *image) {
  int width, height;
  ...
  if (cam->cfg->picture_type == "ppm") {
    ...
    save_webp(picture, image, width, height, &(cam->current_image->imgts),
              &(cam->current_image->location));
  } else if (cam->cfg->picture_type == "grey") {
    save_grey(picture, image, width, height, &(cam->current_image->imgts),
              &(cam->current_image->location));
  } else {
    save_yuv420p(picture, image, width, height, &(cam->current_image->imgts),
                 &(cam->current_image->location));
  }
}

/* Saves image to a file in format requested */
void ImageCapture::save_norm(char *file, u_char *image) {
  FILE *picture;

  picture = myfopen(file, "wbe");
  if (!picture) {
    MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO, _("Can't write picture to file %s"),
               file);
    return;
  }
...
/* Saves image to a file in format requested */
void ImageCapture::save_roi(char *file, u_char *image) {
  FILE *picture;
  ...
  for (indxh = bx->miny; indxh < bx->miny + bx->height; indxh++) {
    memcpy(img + ((indxh - bx->miny) * bx->width),
           image + (indxh * cam->imgs.width) + bx->minx, (uint)bx->width);
  }

  sz = jpgutl_put_grey(buf, image_size, img, bx->width, bx->height,
...
u_char *ImageCapture::load_pgm(FILE *picture, int width, int height) {
  ...
  if (sscanf(line, "%d %d", &mask_width, &mask_height) != 2) {
    MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO, _("Failed reading size in pgm file"));
    return NULL;
  }
  ...
  */
  ...
  for (y = 0; y < mask_height; y++) {
    if ((int)fread(&image[y * mask_width], 1, (uint)mask_width, picture) !=
        mask_width) {
      MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO,
                 "Failed reading image data from pgm file");
    }

    for (x = 0; x < mask_width; x++) {
      image[y * mask_width + x] =
          (u_char)((int)image[y * mask_width + x] * 255 / maxval);
    }
  }
  ...
  if (mask_width != width || mask_height != height) {
    MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO,
               _("The mask file specified is not the same size as image from "
                 "camera."));
    MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO,
    ...
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        resized_image[y * width + x] =
            image[(mask_height - 1) * y / (height - 1) * mask_width +
...
void ImageCapture::write_mask(const char *file) {
  FILE *picture;

  picture = myfopen(file, "wbe");
  if (!picture) {
    /* Report to syslog - suggest solution if the problem is access rights to
     * target dir. */
    if (errno == EACCES) {
      MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO,
                 _("can't write mask file %s - check access rights to target "
                   "directory"),
                 file);
    } else {
      /* If target dir is temporarily unavailable we may survive. */
      MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO, _("can't write mask file %s"),
                 file);
    }
    return;
  }
  memset(cam->imgs.image_motion.image_norm, 255,
         (uint)cam->imgs.motionsize); /* Initialize to unset */
  ...
  if ((int)fwrite(cam->imgs.image_motion.image_norm, (uint)cam->cfg->width,
                  (uint)cam->cfg->height, picture) != cam->cfg->height) {
    MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO,
  ...
  MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
             _("Creating empty mask %s\nPlease edit this file and "
               "re-run motion to enable mask feature"),
             cam->cfg->mask_file.c_str());
}

void ImageCapture::scale_img(int width_src, int height_src, u_char *img_src,
                             u_char *img_dst) {
  ...
  for (y = 0; y < height_src / 2; y += 2)
    for (x = 0; x < width_src; x += 4) {
      ...
      img_dst[i++] =
          img_src[(width_src * height_src) + (y * width_src) + (x + 1)];
    }

  return;
}

void ImageCapture::save_preview() {
  ...
  memcpy(cam->imgs.image_preview.image_norm, cam->current_image->image_norm,
         (uint)cam->imgs.size_norm);
  if (cam->imgs.size_high > 0) {
    memcpy(cam->imgs.image_preview.image_high, cam->current_image->image_high,
           (uint)cam->imgs.size_high);
  }

  /*
   * If we set output_all to yes and during the event
   * there is no image with motion, diffs is 0, we are not going to save the
   * preview event
   */
...
void ImageCapture::init_privacy() {
  int indxrow, indxcol;
  ...
  if (cam->cfg->mask_privacy != "") {
    if ((picture = myfopen(cam->cfg->mask_privacy.c_str(), "rbe"))) {
      ...
      /*
       * NOTE: The mask is expected to have the output dimensions. I.e., the
       * mask applies to the already rotated image, not the capture image. Thus,
       * use width and height from imgs.
       */
      cam->imgs.mask_privacy =
          load_pgm(picture, cam->imgs.width, cam->imgs.height);

      /* We only need the "or" mask for the U & V chrominance area.  */
      cam->imgs.mask_privacy_uv =
          (u_char *)mymalloc((uint)((cam->imgs.height * cam->imgs.width) / 2));
      if (cam->imgs.size_high > 0) {
        MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO,
                   _("Opening high resolution privacy mask file"));
        rewind(picture);
        cam->imgs.mask_privacy_high =
            load_pgm(picture, cam->imgs.width_high, cam->imgs.height_high);
        cam->imgs.mask_privacy_high_uv = (u_char *)mymalloc(
            (uint)((cam->imgs.height_high * cam->imgs.width_high) / 2));
      }

      myfclose(picture);
    } else {
      MOTPLS_LOG(ERR, TYPE_ALL, SHOW_ERRNO, _("Error opening mask file %s"),
                 cam->cfg->mask_privacy.c_str());
      /* Try to write an empty mask file to make it easier for the user to edit
       * it */
    ...
    if (!cam->imgs.mask_privacy) {
      MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
                 _("Failed to read mask privacy image. Mask privacy feature "
                   "disabled."));
    } else {
      MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, _("Mask privacy file \"%s\" loaded."),
                 cam->cfg->mask_privacy.c_str());
...
void ImageCapture::init_mask() {
  ...
  if (cam->cfg->mask_file != "") {
    ...
    if (!cam->imgs.mask) {
      ...
      MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, _("Maskfile \"%s\" loaded."),
                 cam->cfg->mask_file.c_str());
    }
  } else {
    cam->imgs.mask = NULL;
...
ImageCapture::ImageCapture(Camera_t *p_cam) {
  cam = p_cam;
  init_mask();
  init_privacy();
}
