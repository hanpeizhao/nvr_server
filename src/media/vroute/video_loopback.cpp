...
typedef struct capent {
  const char *cap;
  unsigned int code;
} capentT;
capentT cap_list[] = {{"V4L2_CAP_VIDEO_CAPTURE", 0x00000001},
...
static int vlp_open_vidpipe(void) {
  ...
  while ((dirp = readdir(dir)) != NULL) {
    if (!strncmp(dirp->d_name, "video", 5)) {
      ...
      retcd =
          snprintf(buffer, sizeof(buffer), "%s%s/name", prefix, dirp->d_name);
      if ((retcd < 0) || (retcd >= (int)sizeof(buffer))) {
        MOTPLS_LOG(NTC, TYPE_VIDEO, SHOW_ERRNO,
                   _("Error specifying buffer: %s"), buffer);
        continue;
      } else {
        MOTPLS_LOG(NTC, TYPE_VIDEO, SHOW_ERRNO, _("Opening buffer: %s"),
                   buffer);
      }
      ...
      if ((fd = open(buffer, O_RDONLY | O_CLOEXEC)) >= 0) {
        ...
        if (strncmp(buffer, "Loopback video device",
                    21)) { /* weird stuff after minor */
          close(fd);
          continue;
        }
        min = atoi(&buffer[21]);
        ...
        if ((retcd < 0) || (retcd >= (int)sizeof(buffer))) {
          ...
          MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("found video device '%s' %d"),
                     buffer, min);
        }
  ...
  if (pipe_fd >= 0) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Opened %s as pipe output"),
               pipepath);
  }
...
static void vlp_show_vcap(struct v4l2_capability *cap) {
  ...
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "cap.card:     %u.%u.%u",
...
static void vlp_show_vfmt(struct v4l2_format *v) {
  ...
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.width:        %d",
             v->fmt.pix.width);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.height:       %d",
             v->fmt.pix.height);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.pixelformat:  %d",
             v->fmt.pix.pixelformat);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.sizeimage:    %d",
             v->fmt.pix.sizeimage);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.field:        %d",
             v->fmt.pix.field);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.bytesperline: %d",
             v->fmt.pix.bytesperline);
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "fmt.pix.colorspace:   %d",
...
int vlp_startpipe(const char *dev_name, int width, int height) {
  ...
  if (dev < 0) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO,
               _("Opening %s as pipe output failed"), dev_name);
    return -1;
  }
...
void vlp_putpipe(Camera_t *cam) {
#if (defined(HAVE_V4L2)) && (!defined(BSD))
  ssize_t retcd;

  if (cam->pipe >= 0) {
    retcd = write(cam->pipe, cam->current_image->image_norm,
  ...
  if (cam->mpipe >= 0) {
    retcd = write(cam->mpipe, cam->imgs.image_motion.image_norm,
                  (uint)cam->imgs.size_norm);
    if (retcd < 0) {
      MOTPLS_LOG(ERR, TYPE_EVENTS, SHOW_ERRNO,
...
void vlp_init(Camera_t *cam) {
  ...
  if (cam->cfg->video_pipe != "") {
    ...
    cam->pipe = vlp_startpipe(cam->cfg->video_pipe.c_str(), cam->imgs.width,
                              cam->imgs.height);
  ...
  if (cam->cfg->video_pipe_motion != "") {
    ...
    /* vid_startpipe should get the output dimensions */
    cam->mpipe = vlp_startpipe(cam->cfg->video_pipe_motion.c_str(),
