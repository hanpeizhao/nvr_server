...
void Sound_t::init_alerts(ctx_snd_alert *tmp_alert) {
...
void Sound_t::edit_alerts() {
  ...
  validids = true;
  for (it_a0 = snd_info->alerts.begin(); it_a0 != snd_info->alerts.end();
       it_a0++) {
    id_chk = it_a0->alert_id;
    id_cnt = 0;
    for (it_a1 = snd_info->alerts.begin(); it_a1 != snd_info->alerts.end();
         it_a1++) {
  ...
  for (it_a0 = snd_info->alerts.begin(); it_a0 != snd_info->alerts.end();
    ...
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  alert_id:            %d",
               it_a0->alert_id);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  alert_nm             %s",
               it_a0->alert_nm.c_str());
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  freq_low:            %.4f",
               it_a0->freq_low);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  freq_high:           %.4f",
               it_a0->freq_high);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  volume_count:        %d",
               it_a0->volume_count);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  volume_level:        %d",
               it_a0->volume_level);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  trigger_threshold:   %d",
               it_a0->trigger_threshold);
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "  trigger_duration:    %d",
               it_a0->trigger_duration);
  }
}

void Sound_t::load_alerts() {
...
void Sound_t::load_params() {
  ...
  for (indx = 0; indx < snd_info->params->params_cnt; indx++) {
    ...
    MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, "%s : %s", itm->param_name.c_str(),
...
void Sound_t::alsa_list_subdev() {
  ctx_snd_alsa *alsa = snd_info->snd_alsa;
  int indx, retcd, cnt;

  MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("Card %i(%s): %s [%s]"), alsa->card_id,
             alsa->device_nm.c_str(), snd_ctl_card_info_get_id(alsa->card_info),
             snd_ctl_card_info_get_name(alsa->card_info));
  MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("  Device %i (%s,%d): %s [%s]"),
             alsa->device_id, alsa->device_nm.c_str(), alsa->device_id,
  ...
  for (indx = 0; indx < cnt; indx++) {
    ...
    if (retcd < 0) {
      ...
      MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("  Subdevice #%i: %s"), indx,
                 snd_pcm_info_get_subdevice_name(alsa->pcm_info));
    }
  }
}

void Sound_t::alsa_list_card() {
  ctx_snd_alsa *alsa = snd_info->snd_alsa;
  int retcd;
  ...
  if (retcd < 0) {
    ...
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("control hardware info (%i): %s"),
               alsa->card_id, snd_strerror(retcd));
    return;
  }
  ...
  while (alsa->device_id >= 0) {
    ...
    if (retcd == 0) {
      ...
      MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
                 _("control digital audio info (%i): %s"), alsa->card_id,
                 snd_strerror(retcd));
    }
    retcd = snd_ctl_pcm_next_device(alsa->ctl_hdl, &alsa->device_id);
    if (retcd < 0) {
...
void Sound_t::alsa_list() {
  ...
  if ((retcd < 0) || (alsa->card_id == -1)) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("no soundcards found..."));
...
void Sound_t::alsa_start() {
  ...
  retcd = snd_pcm_open(&alsa->pcm_dev, cfg->snd_device.c_str(),
                       SND_PCM_STREAM_CAPTURE, 0);
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("error: snd_pcm_open device %s (%s)"),
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
               _("error: snd_pcm_hw_params_malloc(%s)"), snd_strerror(retcd));
    device_status = STATUS_CLOSED;
    return;
  }
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("error: snd_pcm_hw_params_any(%s)"),
               snd_strerror(retcd));
    device_status = STATUS_CLOSED;
    return;
  }

  retcd = snd_pcm_hw_params_set_access(alsa->pcm_dev, hw_params,
  ...
  retcd = snd_pcm_hw_params_set_format(alsa->pcm_dev, hw_params,
  ...
  retcd =
      snd_pcm_hw_params_set_rate_near(alsa->pcm_dev, hw_params, &smpl_rate, 0);
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
  ...
  retcd = snd_pcm_hw_params_set_channels(alsa->pcm_dev, hw_params,
                                         (uint)snd_info->channels);
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
  ...
  retcd = snd_pcm_hw_params_set_period_size_near(alsa->pcm_dev, hw_params,
                                                 &frames_per, NULL);
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("error: snd_pcm_hw_params(%s)"),
               snd_strerror(retcd));
    device_status = STATUS_CLOSED;
    return;
  }
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("error: snd_pcm_prepare(%s)"),
               snd_strerror(retcd));
    device_status = STATUS_CLOSED;
    return;
  }
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
               _("error: snd_pcm_hw_params_get_rate(%s)"), snd_strerror(retcd));
    device_status = STATUS_CLOSED;
    return;
  }
  ...
  snd_info->buffer =
      (int16_t *)mymalloc((uint)snd_info->buffer_size * sizeof(int16_t));
  memset(snd_info->buffer, 0x00, (uint)snd_info->buffer_size * sizeof(int16_t));

  MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, "Started.");
  device_status = STATUS_OPENED;
}

void Sound_t::alsa_init() {
  ctx_snd_alsa *alsa = snd_info->snd_alsa;
...
void Sound_t::alsa_capture() {
  ...
  retcd =
      snd_pcm_readi(alsa->pcm_dev, snd_info->buffer, (uint)snd_info->frames);
...
void Sound_t::pulse_init() {
  ...
  snd_info->snd_pulse->dev = pa_simple_new(
      ...
      "ztnvr", PA_STREAM_RECORD,
      (cfg->snd_device == "" ? NULL : cfg->snd_device.c_str()), "ztnvr", &specs,
      NULL, NULL, &errcd);
  if (snd_info->snd_pulse->dev == NULL) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("Error opening pulse (%s)"),
               pa_strerror(errcd));
    device_status = STATUS_CLOSED;
    return;
  }
...
void Sound_t::pulse_capture() {
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("Error capturing PulseAudio (%s)"),
               pa_strerror(errcd));
    device_status = STATUS_CLOSED;
  }
}

void Sound_t::pulse_cleanup() {
  if (snd_info->source != "pulse") {
    return;
  }
...
void Sound_t::fftw_open() {
  ...
  MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("Opening FFTW plan"));

  fftw->ff_in =
      (double *)fftw_malloc(sizeof(fftw_complex) * (uint)snd_info->frames);
  fftw->ff_out = (fftw_complex *)fftw_malloc(sizeof(fftw_complex) *
                                             (uint)snd_info->frames);
  fftw->ff_plan = fftw_plan_dft_r2c_1d(snd_info->frames, fftw->ff_in,
                                       fftw->ff_out, FFTW_MEASURE);
...
float Sound_t::HammingWindow(int n1, int N2) {
  return 0.54F - 0.46F * (float)(cos((2 * M_PI * n1)) / (N2 - 1));
}

float Sound_t::HannWindow(int n1, int N2) {
  return 0.5F * (float)(1 - (cos(2 * M_PI * n1 * N2)));
}

void Sound_t::check_alerts() {
  double freq_value;
  ...
  for (indx = snd_info->snd_fftw->bin_min; indx <= snd_info->snd_fftw->bin_max;
       indx++) {
  ...
  freq_value =
      (snd_info->snd_fftw->bin_size * pMaxBinIndex * snd_info->channels);

  if (cfg->snd_show) {
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO,
               _("Freq: %.4f threshold: %d count: %d maximum: %d"), freq_value,
               snd_info->vol_min, snd_info->vol_count, snd_info->vol_max);
  }
    ...
    if (trigger) {
      ...
      if (it->trigger_count == it->trigger_threshold) {
        MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO,
                   ...
                   it->alert_id, it->alert_nm.c_str(), it->volume_level, chkcnt,
                   snd_info->vol_max);
...
void Sound_t::init() {
  ...
  if ((snd_info->source != "alsa") && (snd_info->source != "pulse")) {
...
void Sound_t::check_levels() {
  ...
  for (indx = 0; indx < snd_info->frames; indx++) {
    chkval = abs((int)snd_info->buffer[indx] / 256);
    if (chkval > snd_info->vol_max)
      snd_info->vol_max = chkval;
    if (chkval > snd_info->vol_min)
      snd_info->vol_count++;
  }
...
void Sound_t::handler_startup() {
  ...
  if (handler_running == false) {
    ...
    handler_stop = false;
    retcd = core::threadmgr::spawn_detached(handler_thread, &sound_handler, this);
    if (retcd != 0) {
      MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO,
                 _("Unable to start sound frequency detection loop."));
      handler_running = false;
      handler_stop = true;
    }
  }
