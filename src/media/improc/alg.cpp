 ...
 */
int Algorithm::iflood(int x, int y, int width, int height, u_char *out,
                      int *labels, int newvalue, int oldvalue) {
  int l, x1, x2, dy;
  ...
  while (sp > stack) {
     ...
     */
    for (x = x1;
         x >= 0 && out[y * width + x] != 0 && labels[y * width + x] == oldvalue;
         x--) {
    ...
    do {
      for (; x < width && out[y * width + x] != 0 &&
             labels[y * width + x] == oldvalue;
           x++) {
        labels[y * width + x] = newvalue;
        count++;
      }
    ...
    skip:
      for (x++; x <= x2 &&
                !(out[y * width + x] != 0 && labels[y * width + x] == oldvalue);
           x++)
        ;
      l = x;
    } while (x <= x2);
    ...
    for (ix = 0; ix < width - 1; ix++, pixelpos++) {
      ...
      if (labelsize > 0) {
        ...
        if (labelsize > cam->threshold) {
          labelsize = iflood(ix, iy, width, height, out, labels,
                             current_label + 32768, current_label);
...
/**  Dilates a 3x3 box. */
int Algorithm::dilate9(u_char *img, int width, int height, void *buffer) {
  /*
...
/**  Dilates a + shape. */
int Algorithm::dilate5(u_char *img, int width, int height, void *buffer) {
  /*
   * - row1, row2 and row3 represent lines in the temporary buffer.
...
/**  Erodes a 3x3 box. */
int Algorithm::erode9(u_char *img, int width, int height, void *buffer,
                      u_char flag) {
  int y, i, sum = 0;
  char *Row1, *Row2, *Row3;
  ...
  for (y = 0; y < height; y++) {
    ...
    for (i = width - 2; i >= 1; i--) {
      if (Row1[i - 1] == 0 || Row1[i] == 0 || Row1[i + 1] == 0 ||
          Row2[i - 1] == 0 || Row2[i] == 0 || Row2[i + 1] == 0 ||
          Row3[i - 1] == 0 || Row3[i] == 0 || Row3[i + 1] == 0) {
        img[y * width + i] = 0;
      } else {
        sum++;
      }
...
/* Erodes in a + shape. */
int Algorithm::erode5(u_char *img, int width, int height, void *buffer,
                      u_char flag) {
  int y, i, sum = 0;
  char *Row1, *Row2, *Row3;
  ...
  for (y = 0; y < height; y++) {
    ...
    for (i = width - 2; i >= 1; i--) {
      if (Row1[i] == 0 || Row2[i - 1] == 0 || Row2[i] == 0 ||
          Row2[i + 1] == 0 || Row3[i] == 0) {
        img[y * width + i] = 0;
      } else {
        sum++;
...
void Algorithm::tune_smartmask() {
  ...
  if ((cam->cfg->smart_mask_speed == 0) ||
      (cam->event_curr_nbr == cam->event_prev_nbr) || (--smartmask_count)) {
    return;
  }
...
void Algorithm::lightswitch() {
  if (cam->cfg->lightswitch_percent >= 1) {
    if (cam->current_image->diffs >
        (cam->imgs.motionsize * cam->cfg->lightswitch_percent / 100)) {
...
void Algorithm::ref_frame_update() {
  int accept_timer;
  int i, threshold_ref;
    ...
    if (((int)(abs(*ref - *image_virgin)) > threshold_ref) && (*mask_final)) {
      if (*ref_dyn == 0) { /* Always give new pixels a chance. */
        *ref_dyn = 1;
                 ...
                 accept_timer) { /* Include static Object after some time. */
        *ref_dyn = 0;
        *ref = *image_virgin;
      } else if (*out) {
...
void Algorithm::ref_frame_reset() {
  /* Copy fresh image */
...
void Algorithm::location_center() {
...
void Algorithm::location_dist_stddev() {
  ctx_images *imgs = &cam->imgs;
  int width = cam->imgs.width;
  int height = cam->imgs.height;
  ctx_coord *cent = &cam->current_image->location;
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      if (*(out++)) {
        ...
        distance_mean += (int64_t)sqrt(((x - cent->x) * (x - cent->x)) +
                                       ((y - cent->y) * (y - cent->y)));
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      if (*(out++)) {
        variance_xy += (((int64_t)sqrt(((x - cent->x) * (x - cent->x)) +
                                       ((y - cent->y) * (y - cent->y))) -
                         distance_mean) *
...
void Algorithm::location_dist_basic() {
  ctx_images *imgs = &cam->imgs;
  int width = cam->imgs.width;
  int height = cam->imgs.height;
  ctx_coord *cent = &cam->current_image->location;
...
void Algorithm::location_minmax() {
...
Algorithm::Algorithm(Camera_t *p_cam) {
  int i;

  cam = p_cam;

  if ((cam->cfg->threshold_sdevx == 0) && (cam->cfg->threshold_sdevy == 0) &&
  ...
  smartmask_buffer =
      (int *)mymalloc((uint)cam->imgs.motionsize * sizeof(*smartmask_buffer));

  memset(smartmask, 0, (uint)cam->imgs.motionsize);
  memset(smartmask_final, 255, (uint)cam->imgs.motionsize);
  memset(smartmask_buffer, 0,
         (uint)cam->imgs.motionsize * sizeof(*smartmask_buffer));
