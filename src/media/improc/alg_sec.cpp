...
static void *algsec_handler(void *arg) {
  ((AlgorithmSecondary_t *)arg)->handler();
  return nullptr;
}

void AlgorithmSecondary_t::debug_notice(Mat &mat_dst, bool isdetect) {
  ...
  if (cfg_log_level >= DBG) {
    ...
    if (isdetect == true) {
      imwrite(cfg_target_dir + "/detect_" + method + ".jpg", mat_dst);
    } else {
...
void AlgorithmSecondary_t::image_show(Mat &mat_dst) {
  ...
  if ((cam->stream.secondary.jpg_cnct > 0) || (cam->imgs.size_secondary == 0) ||
      (cfg_log_level >= DBG)) {
...
void AlgorithmSecondary_t::label_image(Mat &mat_dst, std::vector<Rect> &src_pos,
...
void AlgorithmSecondary_t::label_image(Mat &mat_dst, double confidence,
                                       Point classIdPoint) {
  std::string label;
  ...
  try {
    ...
    putText(mat_dst, label, Point(0, 15), FONT_HERSHEY_SIMPLEX, 0.5,
            Scalar(0, 255, 0));
...
void AlgorithmSecondary_t::get_image_roi(Mat &mat_src, Mat &mat_dst) {
  cv::Rect roi;
...
void AlgorithmSecondary_t::get_image(Mat &mat_dst) {
  Mat mat_src;

  if (image_type == "grey") {
    mat_dst =
        Mat(cam->imgs.height, cam->imgs.width, CV_8UC1, (void *)image_norm);
    ...
    if (image_type == "roi") {
      mat_src = Mat(cam->imgs.height * 3 / 2, cam->imgs.width, CV_8UC1,
                    (void *)image_norm);
...
void AlgorithmSecondary_t::detect_hog() {
  ...
  try {
    ...
    hog.detectMultiScale(mat_dst, detect_pos, detect_weights, 0,
                         ...
                         Size(hog_padding, hog_padding), scalefactor,
                         hog_threshold_model, false);
...
void AlgorithmSecondary_t::detect_haar() {
  std::vector<double> detect_weights;
  std::vector<Rect> detect_pos;
  ...
  try {
    ...
    haar_cascade.detectMultiScale(mat_dst, detect_pos, levels, detect_weights,
...
void AlgorithmSecondary_t::detect_dnn() {
  ...
  try {
    ...
    Mat blob = blobFromImage(mat_dst, dnn_scale, Size(dnn_width, dnn_height),
                             Scalar());
    net.setInput(blob);
...
void AlgorithmSecondary_t::load_haar() {
  try {
    if (model_file == "") {
      method = "none";
...
void AlgorithmSecondary_t::load_hog() {
  if (image_type == "roi") {
    image_type = "greyroi";
  } else if ((image_type != "grey") && (image_type != "greyroi")) {
    image_type = "grey";
  }
}

void AlgorithmSecondary_t::load_dnn() {
  std::string line;
  std::ifstream ifs;
  ...
  try {
    ...
    net = readNet(model_file, dnn_config, dnn_framework);
    ...
    if (ifs.is_open() == false) {
      method = "none";
      MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("Classes file not found: %s"),
...
void AlgorithmSecondary_t::params_log() {
  ctx_params_item *itm;
  int indx;
  ...
  if (method != "none") {
    for (indx = 0; indx < params->params_cnt; indx++) {
      ...
      MOTPLS_SHT(INF, TYPE_ALL, NO_ERRNO, "%-25s %s", itm->param_name.c_str(),
                 itm->param_value.c_str());
    }
  }
}

void AlgorithmSecondary_t::params_model() {
  ctx_params_item *itm;
  int indx;
...
void AlgorithmSecondary_t::params_defaults() {
...
void AlgorithmSecondary_t::load_params() {
...
void AlgorithmSecondary_t::handler() {
  long interval;
...
void AlgorithmSecondary_t::handler_startup() {
  int retcd;
  ...
  if (handler_running == false) {
    handler_running = true;
    handler_stop = false;
    retcd = core::threadmgr::spawn_detached(handler_thread, &algsec_handler,
                                            this);
    if (retcd != 0) {
      MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO,
                 _("Unable to start secondary detection"));
      handler_running = false;
      handler_stop = true;
    }
  }
...
void AlgorithmSecondary_t::handler_shutdown() {
  int waitcnt;
...
void AlgorithmSecondary_t::detect() {
#ifdef HAVE_OPENCV
  if (method == "none") {
    return;
  }
    ...
    if (in_process) {
      ...
      if (frame_missed > 10) {
        if (too_slow == 0) {
          ...
          MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO, _("Your computer is too slow."));
        }
        too_slow++;
      }
      frame_missed = 0;
...
AlgorithmSecondary_t::AlgorithmSecondary_t(Camera_t *p_cam) {
#ifdef HAVE_OPENCV
  cam = p_cam;
  handler_running = false;
  handler_stop = true;
...
AlgorithmSecondary_t::~AlgorithmSecondary_t() {
#ifdef HAVE_OPENCV
  handler_shutdown();
