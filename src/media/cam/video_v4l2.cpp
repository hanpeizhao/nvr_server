...
#include "platform/utils/video_convert.hpp"
...
void V4L2Cam::palette_add(uint p_v4l2id) {
  char tmp4cc[5];
  ctx_palette_item p_itm;

  sprintf(tmp4cc, "%c%c%c%c", p_v4l2id >> 0, p_v4l2id >> 8, p_v4l2id >> 16,
          p_v4l2id >> 24);
...
int V4L2Cam::xioctl(unsigned long request, void *arg) {
  int retcd;
...
void V4L2Cam::device_close() {
  close(fd_device);
  fd_device = -1;
}

/* Print the device controls to the log */
void V4L2Cam::ctrls_log() {
  int indx;
    ...
    for (indx = 0; indx < device_ctrls.size(); indx++) {
      ...
      if (itm->ctrl_menuitem) {
        ...
        MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "%s : %s : %d to %d",
                   itm->ctrl_iddesc.c_str(), itm->ctrl_name.c_str(),
                   itm->ctrl_minimum, itm->ctrl_maximum);
      }
    }
      ...
      if ((itm_d->ctrl_iddesc == itm_p->param_name) ||
        ...
        case V4L2_CTRL_TYPE_INTEGER:
          if (mtoi(itm_p->param_value.c_str()) < itm_d->ctrl_minimum) {
            MOTPLS_LOG(
                WRN, TYPE_VIDEO, NO_ERRNO,
                _("%s control option value %s is below minimum.  Skipping..."),
                itm_d->ctrl_name.c_str(), itm_p->param_value.c_str(),
                itm_d->ctrl_minimum);
...
void V4L2Cam::set_input() {
  ...
  if (xioctl(VIDIOC_ENUMINPUT, &input) == -1) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO,
               _("Unable to query input %d."
                 " VIDIOC_ENUMINPUT, if you use a WEBCAM change input value in "
                 "conf by -1"),
               input.index);
    device_close();
    return;
  ...
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO,
             _("Name = \"%s\", type 0x%08X, status %08x"), input.name,
             input.type, input.status);

  if (input.type & V4L2_INPUT_TYPE_TUNER) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Name = \"%s\",- TUNER"),
               input.name);
  }

  if (input.type & V4L2_INPUT_TYPE_CAMERA) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Name = \"%s\"- CAMERA"),
               input.name);
  }
  ...
  if (std_id) {
    ...
    while (xioctl(VIDIOC_ENUMSTD, &standard) == 0) {
      if (standard.id & std_id) {
        MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("- video standard %s"),
                   standard.name);
      }
      standard.index++;
    }
...
void V4L2Cam::set_frequency() {
  ...
  if (device_type & V4L2_INPUT_TYPE_TUNER) {
    ...
    if (xioctl(VIDIOC_G_TUNER, &tuner) == -1) {
      MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO, _("tuner %d VIDIOC_G_TUNER"),
                 tuner.index);
      return;
    }
    ...
    if (xioctl(VIDIOC_S_FREQUENCY, &freq) == -1) {
      MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO, _("freq %ul VIDIOC_S_FREQUENCY"),
                 freq.frequency);
      return;
    }

    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Set Frequency to %ul"),
               freq.frequency);
  }
...
int V4L2Cam::pixfmt_try(uint pixformat) {
  int retcd;
  ...
  if ((retcd == -1) || (vidfmt.fmt.pix.pixelformat != pixformat)) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO,
               _("Unable to use palette %c%c%c%c (%dx%d)"), pixformat >> 0,
               pixformat >> 8, pixformat >> 16, pixformat >> 24,
               cam->cfg->width, cam->cfg->height);
    return -1;
  }

  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Testing palette %c%c%c%c (%dx%d)"),
...
int V4L2Cam::pixfmt_stride() {
  ...
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO,
             _("Checking image size %dx%d with stride %d"), cam->cfg->width,
             cam->cfg->height, bpl);
  ...
  MOTPLS_LOG(WRN, TYPE_VIDEO, NO_ERRNO,
             _("The image width(%d) is not multiple of the stride(%d)"), wd,
             bpl);
...
int V4L2Cam::pixfmt_adjust() {
  if ((vidfmt.fmt.pix.width != (uint)cam->cfg->width) ||
      (vidfmt.fmt.pix.height != (uint)cam->cfg->height)) {

    MOTPLS_LOG(WRN, TYPE_VIDEO, NO_ERRNO,
               _("Adjusting resolution from %ix%i to %ix%i."), cam->cfg->width,
               cam->cfg->height, vidfmt.fmt.pix.width, vidfmt.fmt.pix.height);
...
int V4L2Cam::pixfmt_set(uint pixformat) {
  int retcd;
  ...
  if (retcd == -1) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO, _("Error setting pixel format."));
    return -1;
  }

  pixfmt_src = (int)pixformat;

  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Using palette %c%c%c%c (%dx%d)"),
             pixformat >> 0, pixformat >> 8, pixformat >> 16, pixformat >> 24,
             cam->cfg->width, cam->cfg->height);
...
void V4L2Cam::params_check() {
  ...
  if (cam->cfg->width % 8) {
    ...
    MOTPLS_LOG(WRN, TYPE_VIDEO, NO_ERRNO, _("Adjusting to width (%d)"),
               cam->cfg->width);
  }
  ...
  if (cam->cfg->height % 8) {
    ...
    MOTPLS_LOG(WRN, TYPE_VIDEO, NO_ERRNO, _("Adjusting to height (%d)"),
               cam->cfg->height);
  }
...
/*List camera palettes and return index of one that ztnvr supports*/
int V4L2Cam::pixfmt_list() {
  ...
  while (xioctl(VIDIOC_ENUM_FMT, &fmtd) != -1) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "(%i) %c%c%c%c (%s)", v4l2_pal,
...
void V4L2Cam::palette_set() {
  ...
  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO,
             ...
             indxp, palette[(uint)indxp].fourcc.c_str(), cam->cfg->width,
             cam->cfg->height);
  ...
  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Selected palette index %d (%s)"),
             indxp, palette[(uint)indxp].fourcc.c_str());
}

/* Set the memory mapping from device to Motion*/
void V4L2Cam::set_mmap() {
  ...
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, _("mmap information: frames=%d"),
             buffer_count);
  ...
  for (buffer_index = 0; buffer_index < buffer_count; buffer_index++) {
    ...
    buffers[buffer_index].ptr =
        (unsigned char *)mmap(nullptr, p_buf.length, PROT_READ | PROT_WRITE,
                              MAP_SHARED, fd_device, p_buf.m.offset);

    if (buffers[buffer_index].ptr == MAP_FAILED) {
      MOTPLS_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO, _("Error mapping buffer %i mmap"),
                 buffer_index);
      myfree(buffers);
      device_close();
      return;
    ...
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, _("%i length=%d Address (%x)"),
...
void V4L2Cam::set_imgs() {
  ...
  convert =
      new VideoConvert(cam, pixfmt_src, cam->cfg->width, cam->cfg->height);
}
...
/* Open the device */
void V4L2Cam::device_open() {
  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Opening video device %s"),
             cam->cfg->v4l2_device.c_str());

  cam->watchdog =
      cam->cfg->watchdog_tmo *
      3; /* 3 is arbritrary multiplier to give open function more time*/
  fd_device = open(cam->cfg->v4l2_device.c_str(), O_RDWR | O_CLOEXEC);
  if (fd_device <= 0) {
    MOTPLS_LOG(ALR, TYPE_VIDEO, SHOW_ERRNO, _("Failed to open video device %s"),
               cam->cfg->v4l2_device.c_str());
    fd_device = -1;
    return;
  }
  ...
  if (!(vidcap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO,
               _("Device does not support capturing."));
    device_close();
    return;
  }
}

void V4L2Cam::log_types() {
  if (fd_device == -1) {
    return;
  }
  ...
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "cap.bus_info: \"%s\"",
             vidcap.bus_info);
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "cap.capabilities=0x%08X",
             vidcap.capabilities);
  MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "------------------------");
...
void V4L2Cam::log_formats() {
  ...
  while (xioctl(VIDIOC_ENUM_FMT, &dev_format) != -1) {
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, _("Supported palette %s (%c%c%c%c)"),
               dev_format.description, dev_format.pixelformat >> 0,
               dev_format.pixelformat >> 8, dev_format.pixelformat >> 16,
    ...
    while (xioctl(VIDIOC_ENUM_FRAMESIZES, &dev_sizes) != -1) {
      MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, _("  Width: %d, Height %d"),
                 dev_sizes.discrete.width, dev_sizes.discrete.height);
      ...
      while (xioctl(VIDIOC_ENUM_FRAMEINTERVALS, &dev_frameint) != -1) {
        MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, _("    Framerate %d/%d"),
...
void V4L2Cam::set_fps() {
  ...
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, _("Trying to set fps to %d"),
  ...
  MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, _("Device set fps to %d"),
             setfps.parm.capture.timeperframe.denominator);
}

void V4L2Cam::stop_cam() {
  enum v4l2_buf_type p_type;
  int indx;

  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, _("Closing video device %s"),
             cam->cfg->v4l2_device.c_str());
...
int V4L2Cam::next(ctx_image_data *img_data) {
#ifdef HAVE_V4L2
  int retcd;
  ...
  retcd = convert->process(img_data->image_norm, buffers[vidbuf.index].ptr,
...
V4L2Cam::V4L2Cam(Camera_t *p_cam) {
  cam = p_cam;
#ifdef HAVE_V4L2
