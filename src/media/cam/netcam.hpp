...
class NetCam_t {
public:
  NetCam_t(Camera_t *p_cam, bool p_is_high);
  ~NetCam_t();

  Camera_t *cam;
  bool interrupted; /* Boolean for whether interrupt has been tripped */
  enum NETCAM_STATUS
      status; /* Status of whether the camera is connecting, closed, etc*/
  struct timespec ist_tm; /* The time set before calling the av functions */
  struct timespec
      icur_tm; /* Time during the interrupt to determine duration since start*/
  int idur;    /* Seconds permitted before triggering a interrupt */
  std::string
      camera_name; /* The name of the camera as provided in the config file */
  std::string
      cameratype; /* String specifying Normal or High for use in logging */

  pthread_mutex_t mutex;          /* mutex used with conditional waits */
  pthread_mutex_t mutex_transfer; /* mutex used with transferring stream info
                                     for pass-through */
  pthread_mutex_t mutex_pktarray; /* mutex used with the packet array */

  AVFormatContext *transfer_format; /* Format context just for transferring to
                                       pass-through */
  ctx_packet_item
      *pktarray;     /* Pointer to array of packets for passthru processing */
...
private:
  ...
  netcam_buff_ptr
      img_recv; /* The image buffer that is currently being processed */
  netcam_buff_ptr
      img_latest; /* The most recent image buffer that finished processing */
  ...
  std::string
      service; /* String specifying the type of camera http, rtsp, v4l2 */
  ...
  struct timespec
      frame_prev_tm; /* The time set before calling the av functions */
  struct timespec frame_curr_tm; /* Time during the interrupt to determine
