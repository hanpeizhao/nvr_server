...
enum CAPTURE_RESULT { CAPTURE_SUCCESS, CAPTURE_FAILURE, CAPTURE_ATTEMPTED };
...
struct ctx_image_data {
  ...
  unsigned long cent_dist; /* Movement center to img center distance * Note:
                              Dist is calculated distX*distX + distY*distY */
  unsigned int flags;      /* See IMAGE_* defines */
  ctx_coord
      location; /* coordinates for center and size of last motion detection*/
  int total_labels;
};
...
public:
  ...
  Algorithm *alg;
  AlgorithmSecondary_t *algsec;
  Rotate *rotate;
  NetCam_t *netcam;
  NetCam_t *netcam_high;
  ctx_all_loc all_loc;
...
private:
  MovieCapture *movie_norm;
  MovieCapture *movie_motion;
  MovieCapture *movie_timelapse;
  MovieCapture *movie_extpipe;
  V4L2Cam *v4l2cam;
  LibCam *libcam;
