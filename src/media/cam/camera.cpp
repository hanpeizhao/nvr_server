...
#include "media/cam/libcam.hpp"
#include "media/cam/video_v4l2.hpp"
#include "media/vroute/video_loopback.hpp"
#include "media/cam/netcam.hpp"
#include "conf/conf.hpp"
#include "media/improc/alg.hpp"
...
void Camera_t::ring_resize() {
  ...
  MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("Resizing buffer to %d items"),
             new_size);
...
void Camera_t::ring_process_debug() {
  char tmp[32];
  const char *t;
  ...
  draw->text(imgs.image_ring[imgs.ring_out].image_norm, imgs.width, imgs.height,
             10, 20, tmp, text_scale);
  draw->text(imgs.image_ring[imgs.ring_out].image_norm, imgs.width, imgs.height,
             10, 30, t, text_scale);
}

void Camera_t::ring_process_image() {
  picture->process_norm();
  if (movie_norm->put_image(current_image, &current_image->imgts) == -1) {
    MOTPLS_LOG(ERR, TYPE_EVENTS, NO_ERRNO, _("Error encoding image"));
  }
  if (movie_motion->put_image(&imgs.image_motion, &imgs.image_motion.imgts) ==
      -1) {
    MOTPLS_LOG(ERR, TYPE_EVENTS, NO_ERRNO, _("Error encoding image"));
  }
  if (movie_extpipe->put_image(current_image, &current_image->imgts) == -1) {
    MOTPLS_LOG(ERR, TYPE_EVENTS, NO_ERRNO, _("Error encoding image"));
  }
}

/* Process the entire image ring */
void Camera_t::ring_process() {
  ctx_image_data *saved_current_image = current_image;

  do {
    if ((imgs.image_ring[imgs.ring_out].flags & (IMAGE_SAVE | IMAGE_SAVED)) !=
        IMAGE_SAVE) {
      break;
    }
...
void Camera_t::detected_trigger() {
  time_t raw_time;
  struct tm evt_tm;
  ...
  if (current_image->flags & IMAGE_TRIGGER) {
    if (event_curr_nbr != event_prev_nbr) {
      ...
      MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO,
                 _("Motion detected - starting event %d"), event_curr_nbr);

      mystrftime(this, text_event_string, sizeof(text_event_string),
      ...
      if ((cfg->picture_output == "first") || (cfg->picture_output == "best") ||
...
void Camera_t::mask_privacy() {
  ...
  int indx_max; /* 1 if we are only doing norm, 2 if we are doing both norm and
                   high */
...
/* Start camera */
void Camera_t::cam_start() {
  watchdog = cfg->watchdog_tmo;
  if (camera_type == CAMERA_TYPE_LIBCAM) {
    libcam = new LibCam(this);
  } else if (camera_type == CAMERA_TYPE_NETCAM) {
    netcam = new NetCam_t(this, false);
    netcam->netcam_start();
    if (cfg->netcam_high_url != "") {
      watchdog = cfg->watchdog_tmo;
      netcam_high = new NetCam_t(this, true);
      netcam_high->netcam_start();
    }
  } else if (camera_type == CAMERA_TYPE_V4L2) {
    v4l2cam = new V4L2Cam(this);
  } else {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, _("No Camera device specified"));
    device_status = STATUS_CLOSED;
  }
  watchdog = cfg->watchdog_tmo;
}
...
int Camera_t::cam_next(ctx_image_data *img_data) {
  int retcd, imgsz;
  ...
  if (camera_type == CAMERA_TYPE_LIBCAM) {
    ...
    if ((retcd == CAPTURE_SUCCESS) && (netcam_high != nullptr)) {
...
void Camera_t::init_camera_type() {
  if (cfg->libcam_device != "") {
    ...
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("Unable to determine camera type"));
...
void Camera_t::init_firstimage() {
  ...
  if ((indx >= 5) || (device_status != STATUS_OPENED)) {
    ...
    for (indx = 0; indx < imgs.ring_size; indx++) {
      memset(imgs.image_ring[indx].image_norm, 0x80, (uint)imgs.size_norm);
    }
  }
...
void Camera_t::check_szimg() {
  ...
  if ((imgs.width < 64) || (imgs.height < 64)) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
               _("Motion only supports width and height greater than or equal "
                 "to 64 %dx%d"),
               imgs.width, imgs.height);
    device_status = STATUS_CLOSED;
  }
...
void Camera_t::init_buffers() {
  ...
  imgs.labelsize = (int *)mymalloc((uint)(imgs.motionsize / 2 + 1) *
                                   sizeof(*imgs.labelsize));
  ...
  imgs.image_secondary =
      (u_char *)mymalloc((uint)(3 * imgs.width * imgs.height));
  if (imgs.size_high > 0) {
...
void Camera_t::init_values() {
  ...
  device_status = STATUS_CLOSED;
  startup_frames =
      (cfg->framerate * 2) + cfg->pre_capture + cfg->minimum_motion_frames;
  ...
  if ((camera_type != CAMERA_TYPE_NETCAM) && (movie_passthrough)) {
...
void Camera_t::init_ref() {
  memcpy(imgs.image_virgin, current_image->image_norm, (uint)imgs.size_norm);

  mask_privacy();

  memcpy(imgs.image_vprvcy, current_image->image_norm, (uint)imgs.size_norm);

  alg->ref_frame_reset();
}

/** clean up all memory etc. from motion init */
void Camera_t::cleanup() {
...
void Camera_t::init_cleandir_default() {
  if ((cleandir->action != "delete") && (cleandir->action != "script")) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
  ...
  if ((cleandir->freq != "hourly") && (cleandir->freq != "daily") &&
      (cleandir->freq != "weekly")) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("Invalid clean directory freq : %s"),
               cleandir->freq.c_str());
    cleandir->freq = "";
  }
  if (cleandir->freq == "") {
  ...
  if ((cleandir->dur_unit != "m") && (cleandir->dur_unit != "h") &&
      (cleandir->dur_unit != "d") && (cleandir->dur_unit != "w")) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
...
void Camera_t::init_cleandir_runtime() {
  ...
  if (cleandir->freq == "hourly") {
    ...
    if ((p_min > 59) || (p_min < 0) || (p_hr > 23) || (p_hr < 0)) {
      MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
  ...
  if (cleandir->action == "delete") {
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO,
               ...
               c_tm.tm_min, cleandir->dur_val, cleandir->dur_unit.c_str(),
...
void Camera_t::init_schedule() {
    ...
    for (indx1 = 0; indx1 < params->params_cnt; indx1++) {
      ...
      if ((pnm == tst) || (pnm == "sun-sat") ||
        ...
        if ((sched_itm.st_hr < 0) || (sched_itm.st_hr > 23) ||
            ...
            (sched_itm.st_hr > sched_itm.en_hr) || (pvl.length() != 9)) {
          MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
                     _("Invalid schedule parameter: %s %s"), pnm.c_str(),
                     pvl.c_str());
        } else {
          sched_day.push_back(sched_itm);
        }
  ...
  for (indx = 0; indx < 7; indx++) {
    ...
    for (indx1 = 0; indx1 < schedule[indx].size(); indx1++) {
      ...
      MOTPLS_LOG(
          INF, TYPE_ALL, NO_ERRNO, _("Schedule: %s %02d:%02d to %02d:%02d %s"),
          tst.c_str(), sched_day[indx1].st_hr, sched_day[indx1].st_min,
          sched_day[indx1].en_hr, sched_day[indx1].en_min, action.c_str());
    }
  }
...
void Camera_t::init() {
  ...
  algsec = new AlgorithmSecondary_t(this);
  picture = new ImageCapture(this);
  draw = new Draw(this);
  movie_norm = new MovieCapture(this, "norm");
  movie_motion = new MovieCapture(this, "motion");
  movie_timelapse = new MovieCapture(this, "timelapse");
  movie_extpipe = new MovieCapture(this, "extpipe");
  ...
  if (device_status == STATUS_OPENED) {
    MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO,
               _("Camera %d started: motion detection %s"), cfg->device_id,
...
void Camera_t::areadetect() {
  int i, j, z = 0;

  if ((cfg->area_detect != "") && (event_curr_nbr != areadetect_eventnbr) &&
      (current_image->flags & IMAGE_TRIGGER)) {
    j = (int)cfg->area_detect.length();
    for (i = 0; i < j; i++) {
      z = cfg->area_detect[(uint)i] -
          49; /* characters are stored as ascii 48-57 (0-9) */
      if ((z >= 0) && (z < 9)) {
        if (current_image->location.x > area_minx[z] &&
          ...
          areadetect_eventnbr =
              event_curr_nbr; /* Fire script only once per event */
          MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, _("Motion in area %d detected."),
                     z + 1);
          break;
        }
      }
...
int Camera_t::capture() {
  ...
  if (retcd == CAPTURE_SUCCESS) {
    ...
    if ((device_status == STATUS_OPENED) &&
        (missing_frame_counter < (cfg->device_tmo * cfg->framerate))) {
      ...
      draw->text(current_image->image_norm, imgs.width, imgs.height, 10,
                 20 * text_scale, tmpout, text_scale);
...
void Camera_t::overlay() {
  ...
  if ((cfg->smart_mask_speed > 0) &&
      ((cfg->picture_output_motion != "off") || cfg->movie_output_motion ||
       (stream.motion.jpg_cnct > 0) || (stream.motion.ts_cnct > 0))) {
    draw->smartmask();
  }
  ...
  if (cfg->text_changes) {
    ...
    draw->text(current_image->image_norm, imgs.width, imgs.height,
               imgs.width - 10, 10, tmp, text_scale);
  }

  if ((stream.motion.jpg_cnct > 0) || (stream.motion.ts_cnct > 0)) {
    sprintf(tmp, "D:%5d L:%3d N:%3d", current_image->diffs,
            current_image->total_labels, noise);
    draw->text(imgs.image_motion.image_norm, imgs.width, imgs.height,
               imgs.width - 10, imgs.height - (30 * text_scale), tmp,
               text_scale);
    sprintf(tmp, "THREAD %d SETUP", threadnr);
    draw->text(imgs.image_motion.image_norm, imgs.width, imgs.height,
               imgs.width - 10, imgs.height - (10 * text_scale), tmp,
               text_scale);
  }
  ...
  if (cfg->text_left != "") {
    ...
    draw->text(current_image->image_norm, imgs.width, imgs.height, 10,
               imgs.height - (10 * text_scale), tmp, text_scale);
  }
...
/* emulate motion */
void Camera_t::actions_emulate() {
  int indx;

  if ((detecting_motion == false) && (movie_norm->is_running == true)) {
    movie_norm->reset_start_time(&current_image->imgts);
  }

  if ((detecting_motion == false) && (movie_motion->is_running == true)) {
...
void Camera_t::actions_motion() {
  ...
  if (frame_count >= cfg->minimum_motion_frames) {
    ...
    /* we have motion in this frame, but not enough frames for trigger. Check
     * postcap */
...
void Camera_t::actions_event() {
  ...
  if (event_stop) {
    if (event_curr_nbr == event_prev_nbr) {
      ...
      if (algsec->detected) {
        MOTPLS_LOG(NTC, TYPE_EVENTS, NO_ERRNO, _("Secondary detect"));
        if (cfg->on_secondary_detect != "") {
          util_exec_command(this, cfg->on_secondary_detect.c_str(), NULL);
        }
      }
      algsec->detected = false;
  ...
  if ((cfg->movie_max_time > 0) && (event_curr_nbr == event_prev_nbr) &&
      ((frame_curr_ts.tv_sec - movie_start_time) >= cfg->movie_max_time) &&
...
void Camera_t::timelapse() {
  ...
  if (cfg->timelapse_interval) {
    ...
    if (shots_mt == 0 && frame_curr_ts.tv_sec % cfg->timelapse_interval <=
                             frame_last_ts.tv_sec % cfg->timelapse_interval) {
      movie_timelapse->start();
      if (movie_timelapse->put_image(current_image, &current_image->imgts) ==
          -1) {
        MOTPLS_LOG(ERR, TYPE_EVENTS, NO_ERRNO, _("Error encoding image"));
      }
    }
    ...
    /*
     * If timelapse movie is in progress but conf.timelapse_interval is zero
     * then close timelapse file This is an important feature that allows manual
     * roll-over of timelapse file using the http remote control via a cron job.
     */
    movie_timelapse->stop();
  }
}
...
void Camera_t::handler_startup() {
  ...
  if (handler_running == false) {
    ...
    handler_stop = false;
    restart = false;
    retcd = core::threadmgr::spawn_detached(handler_thread, &camera_handler,
                                            this);
    if (retcd != 0) {
...
void Camera_t::handler_shutdown() {
  int waitcnt;
...
Camera_t::Camera_t(NvrApp *p_app) {
  app = p_app;
