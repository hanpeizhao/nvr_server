...
void LibCam::log_controls() {
  ...
  MOTPLS_SHT(DBG, TYPE_VIDEO, NO_ERRNO,
...
void LibCam::start_params() {
  ...
  for (indx = 0; indx < params->params_cnt; indx++) {
    ...
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "%s : %s", itm->param_name.c_str(),
               itm->param_value.c_str());
  }
}

int LibCam::start_mgr() {
  int retcd;
  std::string camid;

  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Starting.");

  cam_mgr = std::make_unique<CameraManager_t>();
  retcd = cam_mgr->start();
  if (retcd != 0) {
  ...
  if (cam->cfg->libcam_device == "camera0") {
    if (cam_mgr->cameras().size() == 0) {
      MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "No camera devices found");
      return -1;
    }
...
void LibCam::config_control_item(std::string pname, std::string pvalue) {
  if (pname == "AeEnable") {
...
void LibCam::config_controls() {
  int retcd, indx;

  for (indx = 0; indx < params->params_cnt; indx++) {
    config_control_item(params->params_array[indx].param_name,
                        params->params_array[indx].param_value);
  }

  retcd = config->validate();
  if (retcd == CameraConfiguration::Adjusted) {
    MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO, "Configuration controls adjusted.");
  } else if (retcd == CameraConfiguration::Valid) {
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "Configuration controls valid");
  } else if (retcd == CameraConfiguration::Invalid) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Configuration controls error");
  }
}

void LibCam::config_orientation() {
#if (LIBCAMVER >= 2000)
  int retcd, indx;
  std::string adjdesc;
  ctx_params_item *itm;
  ...
  for (indx = 0; indx < params->params_cnt; indx++) {
    ...
    if (itm->param_name == "orientation") {
      if (itm->param_value == "Rotate0") {
        ...
        MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Invalid Orientation option: %s.",
                   itm->param_value.c_str());
      }
    }
  }
  ...
  if (retcd == CameraConfiguration::Adjusted) {
    ...
    MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO,
               "Configuration orientation adjusted to %s.", adjdesc.c_str());
  } else if (retcd == CameraConfiguration::Valid) {
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "Configuration orientation valid");
  } else if (retcd == CameraConfiguration::Invalid) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Configuration orientation error");
  }
#else
  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Orientation Not available");
#endif
...
int LibCam::start_config() {
  ...
  if (retcd == CameraConfiguration::Adjusted) {
    if (config->at(0).pixelFormat != PixelFormat::fromString("YUV420")) {
      MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Pixel format was adjusted to %s.",
                 config->at(0).pixelFormat.toString().c_str());
      return -1;
    } else {
      MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Configuration adjusted.");
    }
  } else if (retcd == CameraConfiguration::Valid) {
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Configuration is valid");
  } else if (retcd == CameraConfiguration::Invalid) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Error setting configuration");
    return -1;
  }
  ...
  if ((config->at(0).size.width != (uint)cam->cfg->width) ||
    ...
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO,
               "Image size adjusted from %d x %d to %d x %d", cam->cfg->width,
               cam->cfg->height, config->at(0).size.width,
...
int LibCam::req_add(Request *request) {
  int retcd;
  retcd = camera->queueRequest(request);
  return retcd;
}

int LibCam::start_req() {
  int retcd, bytes, indx, width;

  MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO, "Starting.");

  camera->requestCompleted.connect(this, &LibCam::req_complete);
  frmbuf = std::make_unique<FrameBufferAllocator>(camera);

  retcd = frmbuf->allocate(config->at(0).stream());
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Buffer allocation error.");
    return -1;
  }

  std::unique_ptr<Request> request = camera->createRequest();
  if (!request) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Create request error.");
    return -1;
  }
  ...
  if (retcd < 0) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Add buffer for request error.");
    return -1;
  }
  ...
  for (indx = 0; indx < (int)buffer->planes().size(); indx++) {
    ...
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "Plane %d of %d length %d", indx,
               buffer->planes().size(), buffer->planes()[(uint)indx].length);
  }
  ...
  if (bytes > cam->imgs.size_norm) {
    ...
    MOTPLS_LOG(NTC, TYPE_VIDEO, NO_ERRNO,
               "Image size adjusted from %d x %d to %d x %d", cam->imgs.width,
               cam->imgs.height, width, cam->imgs.height);
    cam->imgs.width = width;
    cam->imgs.size_norm = (cam->imgs.width * cam->imgs.height * 3) / 2;
    cam->imgs.motionsize = cam->imgs.width * cam->imgs.height;
  }

  membuf.buf = (uint8_t *)mmap(NULL, (uint)bytes, PROT_READ, MAP_SHARED,
                               plane0.fd.get(), 0);
  membuf.bufsz = bytes;
...
int LibCam::start_capture() {
  ...
  if (retcd) {
    MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Failed to start capture.");
    return -1;
  }
  controls.clear();
  ...
  for (std::unique_ptr<Request> &request : requests) {
    ...
    if (retcd < 0) {
      MOTPLS_LOG(ERR, TYPE_VIDEO, NO_ERRNO, "Failed to queue request.");
      if (started_cam) {
        camera->stop();
      }
      return -1;
...
void LibCam::req_complete(Request *request) {
...
void LibCam::libcam_stop() {
  ...
  if (started_req) {
    camera->requestCompleted.disconnect(this, &LibCam::req_complete);
...
int LibCam::next(ctx_image_data *img_data) {
#ifdef HAVE_LIBCAM
  int indx;
