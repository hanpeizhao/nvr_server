...
#include "media/improc/rotate.hpp"
#include "media/cam/netcam.hpp"
#include "media/vroute/movie.hpp"

static void *netcam_handler(void *arg) {
  ((NetCam_t *)arg)->handler();
  return nullptr;
}

enum AVPixelFormat netcam_getfmt_vaapi(AVCodecContext *avctx,
...
enum AVPixelFormat netcam_getfmt_cuda(AVCodecContext *avctx,
                                      const enum AVPixelFormat *pix_fmts) {
  const enum AVPixelFormat *p;
  (void)avctx;
...
enum AVPixelFormat netcam_getfmt_drm(AVCodecContext *avctx,
                                     const enum AVPixelFormat *pix_fmts) {
  const enum AVPixelFormat *p;
  (void)avctx;
...
int netcam_interrupt(void *ctx) {
  ...
  if (netcam->status == NETCAM_CONNECTED) {
    ...
    if ((netcam->icur_tm.tv_sec - netcam->ist_tm.tv_sec) > netcam->idur) {
      if (netcam->cam->finish == false) {
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Camera (%s) timed out"),
                   netcam->cameratype.c_str(), netcam->camera_name.c_str());
      }
      netcam->interrupted = true;
      return true;
...
bool netcam_filelist_cmp(const ctx_filelist_item &a,
                         const ctx_filelist_item &b) {
  return a.filenm < b.filenm;
}

void NetCam_t::filelist_load() {
  DIR *d;
  ...
  filenbr++;
  if ((filenbr == (int)filelist.size()) || (path == "")) {
    filelist.clear();
    retcd = stat(filedir.c_str(), &sbuf);
    if ((sbuf.st_mode & S_IFREG) && (retcd == 0)) {
      MOTPLS_LOG(DBG, TYPE_NETCAM, NO_ERRNO, _("File specified: %s"),
                 filedir.c_str());
      ...
      MOTPLS_LOG(DBG, TYPE_NETCAM, NO_ERRNO, _("Directory specified: %s"),
                 filedir.c_str());
      ...
      closedir(d);
      std::sort(filelist.begin(), filelist.end(), netcam_filelist_cmp);
    }
    filenbr = 0;
  }
  if (filelist.size() == 0) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Directory/file not found: %s"),
               filedir.c_str());
  } else {
    path = filelist[(uint)filenbr].fullnm;
  }
  MOTPLS_LOG(DBG, TYPE_NETCAM, NO_ERRNO, _("Netcam Path: %s"), path.c_str());
}

void NetCam_t::check_buffsize(netcam_buff_ptr buff, size_t numbytes) {
...
#ifdef HAVE_REGEX_H
char *NetCam_t::url_match(regmatch_t m, const char *input) {
  char *match = NULL;
  int len;
...
void NetCam_t::url_invalid(ctx_url *parse_url) {
  MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
             _("Invalid URL.  Can not parse values."));
...
void NetCam_t::url_parse(ctx_url *parse_url, std::string text_url) {
#ifdef HAVE_REGEX_H
  char *s;
  int i, retcd;
  std::string regstr;
...
#else
  // Android Bionic 无 POSIX regex：手动解析 scheme://[user:pass@]host[:port][/path]
  size_t sep = text_url.find("://");
  if (sep == std::string::npos) {
    url_invalid(parse_url);
    return;
  }

  parse_url->service = text_url.substr(0, sep);

  size_t pos = sep + 3;
  size_t at = text_url.find('@', pos);
  if (at != std::string::npos) {
    parse_url->userpass = text_url.substr(pos, at - pos);
    pos = at + 1;
  }

  size_t slash = text_url.find('/', pos);
  std::string hostport = text_url.substr(
      pos, (slash == std::string::npos) ? std::string::npos : slash - pos);
  if (slash != std::string::npos) {
    parse_url->path = text_url.substr(slash);
  }

  size_t colon = hostport.rfind(':');
  if (colon != std::string::npos) {
    parse_url->host = hostport.substr(0, colon);
    parse_url->port = atoi(hostport.substr(colon + 1).c_str());
  } else {
    parse_url->host = hostport;
  }

  if (parse_url->host.empty() && (parse_url->service != "file")) {
    url_invalid(parse_url);
    return;
  }
#endif
  ...
  if (((parse_url->port == 0) && (parse_url->service != "")) ||
    ...
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Using port number %d"),
               parse_url->port);
  }
...
void NetCam_t::free_pkt() {
  av_packet_free(&packet_recv);
  packet_recv = nullptr;
}

int NetCam_t::check_pixfmt() {
  int retcd = -1;
...
void NetCam_t::context_close() {
  if (swsctx != nullptr)
    sws_freeContext(swsctx);
  if (swsframe_in != nullptr)
    av_frame_free(&swsframe_in);
  if (swsframe_out != nullptr)
    av_frame_free(&swsframe_out);
  if (frame != nullptr)
    av_frame_free(&frame);
  if (pktarray != nullptr)
    pktarray_free();
...
void NetCam_t::pktarray_resize() {
   ...
   * to write out.  And putting a mutex on the array during adding function
   * would slow down the capture thread to the speed of the writing thread.  And
   * that writing thread operates at the user specified FPS which could be
   * really slow
  ...
  newsize =
      (int)(((idnbr_first - idnbr_last) * 1) + ((idnbr - idnbr_last) * 2));
  if (newsize < 30) {
    newsize = 30;
  }
  ...
  if ((pktarray_size < newsize) || (pktarray_size < 30)) {
    ...
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Resized packet array to %d"),
               cameratype.c_str(), newsize);
  }
  pthread_mutex_unlock(&mutex_pktarray);
}

void NetCam_t::pktarray_add() {
  int indx_next;
  int retcd;
  char errstr[128];
  ...
  if ((interrupted) || (retcd < 0)) {
    ...
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:av_copy_packet:%s ,Interrupt:%s"), cameratype.c_str(),
...
int NetCam_t::decode_sw() {
  ...
  if ((interrupted) || (handler_stop) || (retcd < 0)) {
    if (retcd == AVERROR(EAGAIN)) {
      ...
      MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Ignoring packet with invalid data"), cameratype.c_str());
      retcd = 0;
    } else if (retcd < 0) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Rec frame error:%s"),
...
int NetCam_t::decode_vaapi() {
  ...
  if (retcd < 0) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Error getting hw frame buffer"), cameratype.c_str());
...
int NetCam_t::decode_video() {
  int retcd;
  char errstr[128];

  /* The Invalid data problem comes frequently.  Usually at startup of rtsp
   * cameras. We now ignore those packets so this function would need to fail on
   * a different error. We should consider adding a maximum count of these
   * errors and reset every time we get a good image.
   */
  if (handler_stop) {
    return 0;
  }
  ...
  if ((interrupted) || (handler_stop)) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Interrupted or handler_stop on send"), cameratype.c_str());
    return -1;
  }
  ...
  if (retcd < 0 && retcd != AVERROR_EOF) {
    av_strerror(retcd, errstr, sizeof(errstr));
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Error sending packet to codec:%s"), cameratype.c_str(),
               errstr);
    if (service == "file") {
      return 0;
    } else {
...
int NetCam_t::decode_packet() {
  ...
  frame_size = av_image_get_buffer_size((enum AVPixelFormat)frame->format,
  ...
  retcd = av_image_copy_to_buffer(
      (uint8_t *)img_recv->ptr, frame_size, (const uint8_t *const *)frame,
...
void NetCam_t::hwdecoders() {
  ...
  if ((hw_type == AV_HWDEVICE_TYPE_NONE) && (first_image)) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:HW Devices:"),
               cameratype.c_str());
    while ((hw_type = av_hwdevice_iterate_types(hw_type)) !=
      ...
      if ((hw_type == AV_HWDEVICE_TYPE_VAAPI) ||
        ...
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s: %s(available)"),
                   cameratype.c_str(), av_hwdevice_get_type_name(hw_type));
      } else {
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s: %s(not implemented)"),
                   cameratype.c_str(), av_hwdevice_get_type_name(hw_type));
      }
    }
  }
...
void NetCam_t::decoder_error(int retcd, const char *fnc_nm) {
  char errstr[128];
  int indx;

  if (interrupted) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:Interrupted"),
               cameratype.c_str());
  } else {
    if (retcd < 0) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:%s:%s"), cameratype.c_str(),
                 fnc_nm, errstr);
    } else {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:%s:Failed"),
                 cameratype.c_str(), fnc_nm);
    }
  }

  if (decoder_nm != "NULL") {
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Decoder %s did not work."),
...
int NetCam_t::init_vaapi() {
  int retcd, indx;
  AVPixelFormat *pixelformats = nullptr;

  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Initializing vaapi decoder"),
             cameratype.c_str());

  hw_type = av_hwdevice_find_type_by_name("vaapi");
  if (hw_type == AV_HWDEVICE_TYPE_NONE) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
               _("%s:Unable to find vaapi hw device"), cameratype.c_str());
  ...
  hw_frames_ref = av_hwframe_ctx_alloc(codec_context->hw_device_ctx);
  ...
  retcd = av_hwframe_transfer_get_formats(codec_context->hw_frames_ctx,
                                          AV_HWFRAME_TRANSFER_DIRECTION_FROM,
                                          &pixelformats, 0);
  if (retcd < 0) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Error enumerating HW pixel types"), cameratype.c_str());
  ...
  */
  ...
  for (indx = 0; pixelformats[indx] != AV_PIX_FMT_NONE; indx++) {
    ...
    MOTPLS_LOG(DBG, TYPE_NETCAM, NO_ERRNO, _("%s:Available HW pixel type:%s"),
...
int NetCam_t::init_cuda() {
  int retcd;

  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Initializing cuda decoder"),
             cameratype.c_str());

  hw_type = av_hwdevice_find_type_by_name("cuda");
  if (hw_type == AV_HWDEVICE_TYPE_NONE) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
               _("%s:Unable to find cuda hw device"), cameratype.c_str());
    decoder_error(0, "av_hwdevice");
    return -1;
  }
...
int NetCam_t::init_drm() {
  int retcd;

  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Initializing drm decoder"),
             cameratype.c_str());
...
int NetCam_t::init_swdecoder() {
  int retcd;

  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Initializing decoder"),
             cameratype.c_str());

  if (decoder_nm != "NULL") {
    decoder = avcodec_find_decoder_by_name(decoder_nm.c_str());
    if (decoder == nullptr) {
      decoder_error(0, "avcodec_find_decoder_by_name");
    } else {
...
int NetCam_t::open_codec() {
  ...
  retcd = av_find_best_stream(format_context, AVMEDIA_TYPE_AUDIO, -1, -1,
                              nullptr, 0);
  if ((retcd < 0) || (interrupted)) {
  ...
  decoder = nullptr;
  retcd = av_find_best_stream(format_context, AVMEDIA_TYPE_VIDEO, -1, -1,
                              &decoder, 0);
  if ((retcd < 0) || (interrupted)) {
  ...
  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Decoder opened"),
             cameratype.c_str());

  return 0;
}

int NetCam_t::open_sws() {
  if (handler_stop) {
    return -1;
  }
  ...
  if (swsframe_in == nullptr) {
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Unable to allocate swsframe_in."), cameratype.c_str());
    }
    context_close();
    return -1;
  }
  ...
  if (swsframe_out == nullptr) {
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Unable to allocate swsframe_out."), cameratype.c_str());
    }
    context_close();
    return -1;
  }
  ...
  if (check_pixfmt() != 0) {
    ...
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Pixel format %s will be converted."), cameratype.c_str(),
               descr->name);
  }

  swsctx = sws_getContext(frame->width, frame->height,
                          (enum AVPixelFormat)frame->format, imgsize.width,
                          imgsize.height, AV_PIX_FMT_YUV420P, SWS_BICUBIC,
  ...
  swsframe_size = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, imgsize.width,
                                           imgsize.height, 1);
  if (swsframe_size <= 0) {
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
...
int NetCam_t::resize() {
  ...
  retcd = av_image_fill_arrays(
      swsframe_in->data, swsframe_in->linesize, (uint8_t *)img_recv->ptr,
      (enum AVPixelFormat)frame->format, frame->width, frame->height, 1);
  if (retcd < 0) {
    if (status == NETCAM_NOTCONNECTED) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Error allocating picture in:%s"), cameratype.c_str(),
                 errstr);
    }
    context_close();
    return -1;
  ...
  retcd = av_image_fill_arrays(swsframe_out->data, swsframe_out->linesize,
                               buffer_out, AV_PIX_FMT_YUV420P, imgsize.width,
                               imgsize.height, 1);
  if (retcd < 0) {
    if (status == NETCAM_NOTCONNECTED) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Error allocating picture out:%s"), cameratype.c_str(),
                 errstr);
    }
    context_close();
    return -1;
  ...
  retcd = sws_scale(swsctx, (const uint8_t *const *)swsframe_in->data,
                    swsframe_in->linesize, 0, frame->height, swsframe_out->data,
                    swsframe_out->linesize);
  if (retcd < 0) {
    if (status == NETCAM_NOTCONNECTED) {
      av_strerror(retcd, errstr, sizeof(errstr));
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Error resizing/reformatting:%s"), cameratype.c_str(),
                 errstr);
    }
    context_close();
    return -1;
  ...
  retcd = av_image_copy_to_buffer((uint8_t *)img_recv->ptr, swsframe_size,
                                  (const uint8_t *const *)swsframe_out,
                                  swsframe_out->linesize, AV_PIX_FMT_YUV420P,
...
void NetCam_t::pkt_ts() {
  ...
  if (packet_recv->pts == AV_NOPTS_VALUE) {
    ...
    if (usec_ltncy < 0) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:Latency calculation error "),
                 cameratype.c_str());
      usec_ltncy = 0;
    }

    packet_recv->pts =
        connection_pts + av_rescale(usec_ltncy, tbase.den / tbase.num, 1000000);
  }
...
int NetCam_t::read_image() {
  ...
  while ((!haveimage) && (!interrupted)) {
    ...
    if ((interrupted) || (errcnt > 1)) {
      if (interrupted) {
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Interrupted"),
                   cameratype.c_str());
      } else {
        av_strerror(retcd, errstr, sizeof(errstr));
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:av_read_frame:%s"),
                   cameratype.c_str(), errstr);
      }
      free_pkt();
      context_close();
  ...
  if (!(high_resolution && passthrough) &&
    ...
    if ((imgsize.width != frame->width) || (imgsize.height != frame->height) ||
        (check_pixfmt() != 0)) {
      if (resize() < 0) {
        free_pkt();
        context_close();
  ...
  if (format_context->streams[video_stream_index]->avg_frame_rate.den > 0) {
    src_fps =
        (int)((format_context->streams[video_stream_index]->avg_frame_rate.num /
    ...
    if (capture_rate < 1) {
      ...
      if (pts_adj == false) {
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
                   _("%s:capture_rate not specified."), cameratype.c_str());
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Using framerate %d"),
                   cameratype.c_str(), capture_rate);
      }
    }
  }
...
int NetCam_t::ntc() {
  ...
  if ((imgsize.width == 0) || (imgsize.height == 0) || (frame->width == 0) ||
      (frame->height == 0)) {
    return 0;
  }
  ...
  if ((imgsize.width != frame->width) || (imgsize.height != frame->height)) {
    ...
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               "******************************************************");
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               _("The network camera is sending pictures at %dx%d"),
               frame->width, frame->height);
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               _("resolution but config is %dx%d. If possible change"),
               imgsize.width, imgsize.height);
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               _("the netcam or config so that the image height and"));
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               _("width are the same to lower the CPU usage."));
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
               "******************************************************");
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, "");
  }
...
void NetCam_t::set_options() {
  std::string tmp;
  int indx;
  ctx_params_item *itm;

  if ((service == "rtsp") || (service == "rtsps") || (service == "rtmp")) {
    ...
    tmp = std::to_string(cfg_width) + "x" + std::to_string(cfg_height);
  ...
  for (indx = 0; indx < params->params_cnt; indx++) {
    ...
    if ((itm->param_name != "decoder") && (itm->param_name != "capture_rate") &&
        (itm->param_name != "interrupt") &&
        (itm->param_name != "input_format")) {
      av_dict_set(&opts, itm->param_name.c_str(), itm->param_value.c_str(), 0);
      if (status == NETCAM_NOTCONNECTED) {
        MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:%s = %s"),
                   cameratype.c_str(), itm->param_name.c_str(),
...
void NetCam_t::set_path() {
  ...
  if (url.service == "v4l2") {
    path = url.path;
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Setting up v4l2"));
  } else if (url.service == "file") {
    filedir = url.path;
    filelist_load();
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Setting up file"));
  } else {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Setting up %s "),
               url.service.c_str());
    if (url.userpass.length() > 0) {
      path = url.service + "://" + url.userpass + "@" + url.host + ":" +
             std::to_string(url.port) + url.path;
    } else {
      path = url.service + "://" + url.host + ":" + std::to_string(url.port) +
             url.path;
    }
  }
...
void NetCam_t::set_parms() {
  ...
  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Setting up camera(%s)."),
  ...
  /* If this is the norm and we have a highres, then disable passthru on the
   * norm */
  if ((high_resolution == false) && (cam->cfg->netcam_high_url != "")) {
    passthrough = false;
  } else {
...
/* Make a static copy of the stream information for use in passthrough
 * processing */
int NetCam_t::copy_stream() {
  ...
  for (indx = 0; indx < (int)format_context->nb_streams; indx++) {
    if ((audio_stream_index == indx) || (video_stream_index == indx)) {
      transfer_stream = avformat_new_stream(transfer_format, nullptr);
      stream_in = format_context->streams[indx];
      retcd = avcodec_parameters_copy(transfer_stream->codecpar,
                                      stream_in->codecpar);
      if (retcd < 0) {
        MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                   _("%s:Unable to copy codec parameters"), cameratype.c_str());
...
int NetCam_t::open_context() {
  ...
  idur = cfg_idur * 3; /*3 is arbritrary multiplier to give connect more time
                          than other steps*/

  set_options();

  MOTPLS_LOG(DBG, TYPE_NETCAM, NO_ERRNO, _("Opening camera"));
  retcd = avformat_open_input(&format_context, path.c_str(), nullptr, &opts);
  if ((retcd < 0) || (interrupted) || (handler_stop)) {
    if (status == NETCAM_NOTCONNECTED) {
      if (retcd < 0) {
        av_strerror(retcd, errstr, sizeof(errstr));
        MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
                   _("%s:Unable to open camera(%s):%s"), cameratype.c_str(),
  ...
  clock_gettime(CLOCK_MONOTONIC, &ist_tm);
  av_dict_free(&opts);
  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Opened camera(%s)"),
             cameratype.c_str(), camera_name.c_str());
  ...
  if ((retcd < 0) || (interrupted) || (handler_stop)) {
    if (status == NETCAM_NOTCONNECTED) {
      if (retcd < 0) {
        av_strerror(retcd, errstr, sizeof(errstr));
        MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                   _("%s:Unable to find stream info:%s"), cameratype.c_str(),
                   errstr);
      } else if (interrupted) {
        MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                   _("%s:Unable to find stream info:%s"), cameratype.c_str());
  ...
  if ((retcd < 0) || (interrupted) || (handler_stop)) {
    ...
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
                 _("%s:Unable to open codec context:%s"), cameratype.c_str(),
                 errstr);
    } else {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO,
  ...
  if (codec_context->width <= 0 || codec_context->height <= 0) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:Camera image size is invalid"),
               cameratype.c_str());
    context_close();
    return -1;
  }
  ...
  if (frame == nullptr) {
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:Unable to allocate frame."),
                 cameratype.c_str());
    }
    context_close();
    return -1;
  ...
  if ((retcd < 0) || (interrupted)) {
    if (status == NETCAM_NOTCONNECTED) {
      MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("%s:Failed to read first image"),
                 cameratype.c_str());
    }
    context_close();
    return -1;
...
int NetCam_t::connect() {
  ...
  if (!first_image) {
    ...
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Camera (%s) connected"),
               cameratype.c_str(), camera_name.c_str());

    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Netcam capture FPS is %d."),
               cameratype.c_str(), capture_rate);

    if (src_fps > 0) {
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Camera source is %d FPS"),
    ...
    if (audio_stream_index != -1) {
      /* The following is not technically precise but we want to convey in
       * simple terms that the capture rate must go faster to account for the
       * additional packets read for the audio stream.  The technically correct
       * process is that our wait timer in the handler is only triggered when
       * the last packet is a video stream
       */
      MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
                 _("%s:An audio stream was detected.  Capture_rate increased "
                   "to compensate."),
                 cameratype.c_str());
    }
  }
...
void NetCam_t::handler_reconnect() {
  int retcd, slp_dur;

  if (service == "file") {
    filelist_load();
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Processing file: %s"),
               cameratype.c_str(), path.c_str());
  } else if ((status == NETCAM_CONNECTED) || (status == NETCAM_READINGIMAGE)) {
    MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Reconnecting with camera...."),
               cameratype.c_str());
  }
  ...
  if (retcd < 0) {
    if (reconnect_count < 100) {
      ...
      if (reconnect_count >= 500) {
        MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
                   _("%s:Camera did not reconnect."), cameratype.c_str());
        MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
...
void NetCam_t::handler() {
  handler_running = true;

  mythreadname_set("nc", threadnbr, camera_name.c_str());

  MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Camera handler started"),
             cameratype.c_str());
  ...
  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("%s:Camera handler stopped"),
             cameratype.c_str());
  handler_running = false;
  pthread_exit(nullptr);
}

void NetCam_t::handler_startup() {
  int wait_counter, retcd;

  if (handler_running == false) {
    handler_running = true;
    handler_stop = false;
    retcd = core::threadmgr::spawn_detached(handler_thread, &netcam_handler,
                                            this);
    if (retcd != 0) {
...
void NetCam_t::handler_shutdown() {
  int waitcnt;
  ...
  if (handler_running == true) {
    MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO,
               _("%s:Shutting down network camera."), cameratype.c_str());
    waitcnt = 0;
  ...
  status = NETCAM_NOTCONNECTED;
  MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("%s:Shut down complete."),
             cameratype.c_str());
}

void NetCam_t::netcam_start() {
  int retcd;
  ...
  cam->watchdog = cam->cfg->watchdog_tmo *
                  3; /* 3 is arbitrary multiplier to give startup more time*/
  set_parms();
  if (service == "file") {
    retcd = connect();
    while ((retcd != 0) && (filenbr < (int)filelist.size())) {
      filelist_load();
      retcd = connect();
    }
    if (retcd != 0) {
  ...
  cam->watchdog = cam->cfg->watchdog_tmo *
                  3; /* 3 is arbitrary multiplier to give startup more time*/
  /* When running dual, there seems to be contamination across norm/high with
   * codec functions. */
  context_close(); /* Close in this thread to open it again within handler
                      thread */
  status = NETCAM_RECONNECTING; /* Set as reconnecting to avoid excess messages
                                   when starting */
...
void NetCam_t::netcam_stop() { handler_shutdown(); }

void NetCam_t::noimage() {
  int slp_dur;
  ...
  if (handler_running == false) {
    if (reconnect_count >= 500) {
      ...
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
                 _("Checking for camera every 2 hours."));
      slp_dur = 7200;
    } else if (reconnect_count >= 200) {
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("Camera did not reconnect."));
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
                 _("Checking for camera every 10 minutes."));
      reconnect_count++;
      slp_dur = 600;
    } else {
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO, _("Camera did not reconnect."));
      MOTPLS_LOG(NTC, TYPE_NETCAM, NO_ERRNO,
                 _("Checking for camera every 30 seconds."));
      reconnect_count++;
      slp_dur = 30;
    }
    netcam_stop();
...
int NetCam_t::next(ctx_image_data *img_data) {
  if ((status == NETCAM_RECONNECTING) || (status == NETCAM_NOTCONNECTED)) {
  ...
  pktarray_resize();
  if (high_resolution == false) {
    memcpy(img_data->image_norm, img_latest->ptr, img_latest->used);
    img_data->idnbr_norm = idnbr;
  } else {
    img_data->idnbr_high = idnbr;
    if (cam->netcam_high->passthrough == false) {
      memcpy(img_data->image_high, img_latest->ptr, img_latest->used);
    }
  }
  pthread_mutex_unlock(&mutex);
...
NetCam_t::NetCam_t(Camera_t *p_cam, bool p_is_high) {
  cam = p_cam;
