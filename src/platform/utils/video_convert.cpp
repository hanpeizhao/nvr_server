 ...
 *
 */
void VideoConvert::sonix_decompress_init(sonix_table *table) {
  int i;
  int is_abs, val, len;
 ...
 *
 */
int VideoConvert::sonix_decompress(u_char *img_dst, u_char *img_src) {
  int row, col;
  int val;
  int bitpos;
  u_char code;
  ...
  for (row = 0; row < height; row++) {
    ...
    if (row < 2) {
      ...
      code =
          (u_char)((addr[0] << (bitpos & 7)) | (addr[1] >> (8 - (bitpos & 7))));
      bitpos += 8;
      *img_dst++ = code;
 ...
 *
 */
void VideoConvert::bayer2rgb24(u_char *img_dst, u_char *img_src) {
  long int i;
  u_char *rawpt, *scanpt;
  long int size;
    ...
    if (((i / width) & 1) == 0) {
      if ((i & 1) == 0) {
        ...
        if ((i > width) && ((i % width) > 0)) {
          ...
          *scanpt++ = (u_char)((*(rawpt - 1) + *(rawpt + 1) + *(rawpt + width) +
                                *(rawpt - width)) /
                               4); /* G */
          *scanpt++ = (u_char)((*(rawpt - width - 1) + *(rawpt - width + 1) +
                                *(rawpt + width - 1) + *(rawpt + width + 1)) /
                               4); /* R */
        } else {
          /* First line or left column. */
          *scanpt++ = (u_char)(*rawpt);                                /* B */
          *scanpt++ = (u_char)((*(rawpt + 1) + *(rawpt + width)) / 2); /* G */
          *scanpt++ = (u_char)(*(rawpt + width + 1));                  /* R */
        }
      } else {
        /* (B)G */
        if ((i > width) && ((i % width) < (width - 1))) {
          *scanpt++ = (u_char)((*(rawpt - 1) + *(rawpt + 1)) / 2); /* B */
          *scanpt++ = (u_char)(*rawpt);                            /* G */
          *scanpt++ =
              (u_char)((*(rawpt + width) + *(rawpt - width)) / 2); /* R */
        } else {
      ...
      if ((i & 1) == 0) {
        ...
        if ((i < (width * (height - 1))) && ((i % width) > 0)) {
          *scanpt++ =
              (u_char)((*(rawpt + width) + *(rawpt - width)) / 2); /* B */
          *scanpt++ = (u_char)(*rawpt);                            /* G */
          *scanpt++ = (u_char)((*(rawpt - 1) + *(rawpt + 1)) / 2); /* R */
        } else {
...
void VideoConvert::yuv422to420p(u_char *img_dst, u_char *img_src) {
...
void VideoConvert::yuv422pto420p(u_char *img_dst, u_char *img_src) {
...
void VideoConvert::uyvyto420p(u_char *img_dst, u_char *img_src) {
  u_char *pY = img_dst;
...
void VideoConvert::rgb_bgr(u_char *img_dst, u_char *img_src, int rgb) {
  u_char *y, *u, *v;
  u_char *r, *g, *b;
  int i, loop;
...
void VideoConvert::rgb24toyuv420p(u_char *img_dst, u_char *img_src) {
  rgb_bgr(img_dst, img_src, 1);
}

void VideoConvert::bgr24toyuv420p(u_char *img_dst, u_char *img_src) {
 ...
 */
int VideoConvert::mjpegtoyuv420p(u_char *img_dst, u_char *img_src, int size) {
  u_char *ptr_buffer;
  size_t soi_pos = 0;
  int ret = 0;
  ...
  */
  while (ptr_buffer != NULL && (((uint)size - soi_pos - 1) > 2)) {
    ...
    ptr_buffer = (u_char *)memmem(img_src + soi_pos + 1,
                                  (uint)size - soi_pos - 1, "\xff\xd8", 2);
  }

  if (soi_pos != 0) {
    MOTPLS_LOG(INF, TYPE_VIDEO, NO_ERRNO,
               _("SOI position adjusted by %d bytes."), soi_pos);
  }
...
void VideoConvert::y10torgb24(u_char *img_dst, u_char *img_src, int shift) {
  /* Source code: raw2rgbpnm project */
  /* url:
   * http://salottisipuli.retiisi.org.uk/cgi-bin/gitweb.cgi?p=~sailus/raw2rgbpnm.git;a=summary
   */

  /* bpp - bits per pixel */
  /* bpp: 'Pixels are stored in 16-bit words with unused high bits padded with
   * 0' */
  ...
  for (src_y = 0, dst_y = 0; dst_y < src_size[1]; src_y++, dst_y++) {
    for (src_x = 0, dst_x = 0; dst_x < src_size[0]; src_x++, dst_x++) {
      a = (img_src[(uint)(src_y * src_stride + src_x * 2 + 0)] |
           (img_src[(uint)(src_y * src_stride + src_x * 2 + 1)] << 8)) >>
          shift;
...
void VideoConvert::greytoyuv420p(u_char *img_dst, u_char *img_src) {
...
int VideoConvert::process(u_char *img_dst, u_char *img_src, int clen) {
#ifdef HAVE_V4L2
...
VideoConvert::VideoConvert(Camera_t *p_cam, int p_pix, int p_w, int p_h) {
  cam = p_cam;
  width = p_w;
  height = p_h;
  pixfmt_src = p_pix;
