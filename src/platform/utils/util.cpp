  ...
  if (size == 0) {
    ...
    if (!dummy) {
      MOTPLS_LOG(EMG, TYPE_ALL, NO_ERRNO,
                 _("Could not resize memory-block at offset %p to %llu bytes "
                   "(function %s)!"),
...
int mycreate_path(const char *path) {
  ...
  while (indx_pos != std::string::npos) {
    if (stat(tmp.substr(0, indx_pos + 1).c_str(), &statbuf) != 0) {
      MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO, _("Creating %s"),
...
void mystrftime(Sound_t *snd, std::string &dst, std::string fmt) {
  char tmp[PATH_MAX];
...
void mystrftime_base(Camera_t *cam, std::string &dst, std::string fmt,
                     std::string fname) {
  char tmp[PATH_MAX];
  struct tm timestamp_tm;
  timespec curr_ts;
...
void mystrftime(Camera_t *cam, char *s, size_t mx_sz, const char *usrfmt,
                const char *fname) {
...
void mystrftime(Camera_t *cam, std::string &rslt, std::string usrfmt,
                std::string fname) {
...
#else
  MOTPLS_LOG(INF, TYPE_NETCAM, NO_ERRNO, _("Unable to set thread name %s"),
             tname);
#endif
}
...
void mytranslate_init(void) {
  ...
  bindtextdomain("ztnvr", LOCALEDIR);
  bind_textdomain_codeset("ztnvr", "UTF-8");
  textdomain("ztnvr");
  ...
  if (setnls == 0) {
    if (nls_enabled) {
      MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO,
                 _("Disabling native language support"));
    }
    nls_enabled = false;
    return NULL;
    ...
    if (!nls_enabled) {
      MOTPLS_LOG(NTC, TYPE_ALL, NO_ERRNO,
                 _("Enabling native language support"));
    }
    nls_enabled = true;
    return NULL;
...
void util_exec_command(Camera_t *cam, const char *command,
                       const char *filename) {
  char stamp[PATH_MAX];
  int pid;
  ...
  MOTPLS_LOG(DBG, TYPE_EVENTS, NO_ERRNO, _("Executing external command '%s'"),
             stamp);
}

void util_exec_command(Camera_t *cam, std::string cmd) {
  std::string dst;
  int pid;
...
void util_exec_command(Sound_t *snd, std::string cmd) {
  std::string dst;
  int pid;
...
static void util_parms_file(ctx_params *params, std::string params_file) {
  ...
  MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, _("parse file:%s"), params_file.c_str());
  ...
  while (std::getline(ifs, line)) {
    ...
    if ((stpos != std::string::npos) && (stpos != line.length() - 1) &&
        (stpos != 0) && (line.substr(0, 1) != ";") &&
        (line.substr(0, 1) != "#")) {
...
void util_parms_add(ctx_params *params, std::string parm_nm,
                    std::string parm_val) {
  int indx;
...
static void util_parms_extract(ctx_params *params, std::string &parmline,
                               size_t indxnm_st, size_t indxnm_en,
                               size_t indxvl_st, size_t indxvl_en) {
  std::string parm_nm, parm_vl;

  if ((indxnm_st != std::string::npos) && (indxnm_en != std::string::npos) &&
      (indxvl_st != std::string::npos) && (indxvl_en != std::string::npos)) {
...
static void util_parms_next(std::string &parmline, size_t indxnm_st,
                            size_t indxvl_en) {
  size_t indxcm;
  ...
  if (indxnm_st == 0) {
    ...
    if (indxcm == std::string::npos) {
      ...
      parmline =
          parmline.substr(0, indxnm_st - 1) + parmline.substr(indxcm + 1);
    }
  }
  mytrim(parmline);
}
...
void util_parms_parse_qte(ctx_params *params, std::string &parmline) {
  ...
  while (parmline.find("\"", 0) != std::string::npos) {
    ...
    if (indxcm <= indxeq) {
      ...
      if ((indxnm_en == std::string::npos) || (indxnm_en > indxvl_st)) {
    ...
    util_parms_extract(params, parmline, indxnm_st, indxnm_en, indxvl_st,
                       indxvl_en);
...
void util_parms_parse(ctx_params *params, std::string parm_desc,
                      std::string confline) {
  std::string parmline;
...
/* Add the requested int value as a default if the parm_nm does have anything
 * yet */
void util_parms_add_default(ctx_params *params, std::string parm_nm,
                            int parm_vl) {
  bool dflt;
  int indx;
...
/* Add the requested string value as a default if the parm_nm does have anything
 * yet */
void util_parms_add_default(ctx_params *params, std::string parm_nm,
                            std::string parm_vl) {
  bool dflt;
...
/* my to integer*/
int mtoi(std::string parm) { return atoi(parm.c_str()); }
/* my to integer*/
int mtoi(char *parm) { return atoi(parm); }
/* my to long*/
long mtol(std::string parm) { return atol(parm.c_str()); }
/* my to long*/
long mtol(char *parm) { return atol(parm); }
/* my to float*/
float mtof(char *parm) { return (float)atof(parm); }
/* my to float*/
float mtof(std::string parm) { return (float)atof(parm.c_str()); }
/* my to bool*/
bool mtob(std::string parm) {
  if (mystrceq(parm.c_str(), "1") || mystrceq(parm.c_str(), "yes") ||
      mystrceq(parm.c_str(), "on") || mystrceq(parm.c_str(), "true")) {
    return true;
  } else {
    return false;
  }
...
/* my to bool*/
bool mtob(char *parm) {
  if (mystrceq(parm, "1") || mystrceq(parm, "yes") || mystrceq(parm, "on") ||
      mystrceq(parm, "true")) {
    return true;
  } else {
    return false;
...
void util_resize(uint8_t *src, int src_w, int src_h, uint8_t *dst, int dst_w,
                 int dst_h) {
  int retcd, img_sz;
  char errstr[128];
  uint8_t *buf;
  ...
  if (frm_in == NULL) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("Unable to allocate frm_in."));
    return;
  }

  frm_out = av_frame_alloc();
  if (frm_out == NULL) {
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("Unable to allocate frm_out."));
    av_frame_free(&frm_in);
    return;
  }

  retcd = av_image_fill_arrays(frm_in->data, frm_in->linesize, src,
                               AV_PIX_FMT_YUV420P, src_w, src_h, 1);
  if (retcd < 0) {
    av_strerror(retcd, errstr, sizeof(errstr));
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, "Error filling arrays: %s", errstr);
    av_frame_free(&frm_in);
  ...
  retcd = av_image_fill_arrays(frm_out->data, frm_out->linesize, buf,
                               AV_PIX_FMT_YUV420P, dst_w, dst_h, 1);
  if (retcd < 0) {
    av_strerror(retcd, errstr, sizeof(errstr));
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, "Error Filling array 2: %s", errstr);
    free(buf);
    av_frame_free(&frm_in);
    av_frame_free(&frm_out);
    return;
  ...
  swsctx = sws_getContext(src_w, src_h, AV_PIX_FMT_YUV420P, dst_w, dst_h,
  ...
  retcd =
      sws_scale(swsctx, (const uint8_t *const *)frm_in->data, frm_in->linesize,
                0, src_h, frm_out->data, frm_out->linesize);
  if (retcd < 0) {
    av_strerror(retcd, errstr, sizeof(errstr));
    MOTPLS_LOG(ERR, TYPE_NETCAM, NO_ERRNO, _("Error resizing/reformatting: %s"),
               errstr);
    free(buf);
    av_frame_free(&frm_in);
    av_frame_free(&frm_out);
  ...
  retcd = av_image_copy_to_buffer(
      (uint8_t *)dst, img_sz, (const uint8_t *const *)frm_out,
      frm_out->linesize, AV_PIX_FMT_YUV420P, dst_w, dst_h, 1);
