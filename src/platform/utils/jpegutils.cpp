...
static void put_direntry(struct tiff_writing *into, const char *data,
                         uint length) {
  if (length <= 4) {
...
static void put_stringentry(struct tiff_writing *into, uint tag,
                            const char *str, int with_nul) {
...
void jpgutl_exif_date(ctx_exif_info *exif_info) {
  ...
  snprintf(tmpbuf, 45, "%04d:%02d:%02d %02d:%02d:%02d",
           ...
           exif_info->timestamp_tm.tm_mon + 1, exif_info->timestamp_tm.tm_mday,
           exif_info->timestamp_tm.tm_hour, exif_info->timestamp_tm.tm_min,
...
void jpgutl_exif_tags(ctx_exif_info *exif_info) {
  ...
  if (exif_info->description != nullptr) {
    ...
    exif_info->datasize +=
        5 +
        (uint)strlen(exif_info->description); /* Add 5 for NUL and alignment */
  }
...
void jpgutl_exif_writeifd0(ctx_exif_info *exif_info) {
  ...
  if (exif_info->description) {
    put_stringentry(&exif_info->writing, TIFF_TAG_IMAGE_DESCRIPTION,
                    exif_info->description, 1);
  }

  if (exif_info->datetime) {
    put_stringentry(&exif_info->writing, TIFF_TAG_DATETIME, exif_info->datetime,
                    1);
  }
...
void jpgutl_exif_writeifd1(ctx_exif_info *exif_info) {
  ...
  if (exif_info->ifd1_tagcount > 0) {
    ...
    if (exif_info->datetime) {
      put_stringentry(&exif_info->writing, EXIF_TAG_ORIGINAL_DATETIME,
                      exif_info->datetime, 1);
    }
    ...
    if (exif_info->subtime) {
      put_stringentry(&exif_info->writing, EXIF_TAG_ORIGINAL_DATETIME_SS,
                      exif_info->subtime, 0);
    }
...
uint jpgutl_exif(u_char **exif, Camera_t *cam, timespec *ts_in1,
  ...
  exif_info->writing.base =
      marker + 6; /* base address for intra-TIFF offsets */
  exif_info->writing.buf = marker + 14; /* current write position */
  exif_info->writing.data_offset =
      (uint)(8 + exif_info->ifds_size); /* where to start storing data */
...
static void add_huff_table(j_decompress_ptr dinfo, JHUFF_TBL **htblptr,
  ...
  if (nsymbols < 1 || nsymbols > 256) {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
               _("%s: Given jpeg buffer was too small"));
  }
...
static void std_huff_tables(j_decompress_ptr dinfo) {
  ...
  add_huff_table(dinfo, &dinfo->dc_huff_tbl_ptrs[0], bits_dc_luminance,
                 val_dc_luminance);
  add_huff_table(dinfo, &dinfo->ac_huff_tbl_ptrs[0], bits_ac_luminance,
                 val_ac_luminance);
  add_huff_table(dinfo, &dinfo->dc_huff_tbl_ptrs[1], bits_dc_chrominance,
                 val_dc_chrominance);
  add_huff_table(dinfo, &dinfo->ac_huff_tbl_ptrs[1], bits_ac_chrominance,
                 val_ac_chrominance);
}
 ...
 * jpgutl_buffer_src
 *  Purpose:
 *    Establish the input buffer source for the JPEG libary and associated
 * helper functions. Parameters: cinfo      The jpeg library
 * compression/decompression information buffer     The buffer of JPEG data to
 * decompress. buffer_len The length of the buffer. Return values: None
 */
static void jpgutl_buffer_src(j_decompress_ptr cinfo, u_char *buffer,
                              long buffer_len) {
...
static void jpgutl_emit_message(j_common_ptr cinfo, int msg_level) {
  ...
  struct jpgutl_error_mgr *myerr = (struct jpgutl_error_mgr *)cinfo->err;
  /*
   *  The JWRN_EXTRANEOUS_DATA is sent a lot without any particular negative
   * effect. There are some messages above zero but they are just informational
   * and not something that we are interested in.
   */
  if ((cinfo->err->msg_code != JWRN_EXTRANEOUS_DATA) && (msg_level < 0)) {
    myerr->warning_seen++;
    (*cinfo->err->format_message)(cinfo, buffer);
    MOTPLS_LOG(DBG, TYPE_VIDEO, NO_ERRNO, "msg_level: %d, %s", msg_level,
               buffer);
  }
}
...
static GLOBAL(void)
    _jpeg_mem_dest(j_compress_ptr cinfo, JOCTET *buf, size_t bufsize) {
  mem_dest_ptr dest;
 ...
 */
static void put_jpeg_exif(j_compress_ptr cinfo, Camera_t *cam, timespec *ts1,
                          ctx_coord *box) {
  u_char *exif = NULL;
 ...
 *    Success 0, Failure -1
 */
int jpgutl_decode_jpeg(u_char *jpeg_data_in, int jpeg_data_len, uint width,
                       uint height, u_char *volatile img_out) {
  ...
  guarantee_huff_tables(
      &dinfo); /* Required by older versions of the jpeg libs */
  ...
  if ((dinfo.output_width != width) || (dinfo.output_height != height)) {
    MOTPLS_LOG(WRN, TYPE_VIDEO, NO_ERRNO,
               _("JPEG image size %dx%d, JPEG was %dx%d"), width, height,
...
int jpgutl_put_yuv420p(u_char *dest_image, int image_size, u_char *input_image,
                       int width, int height, int quality, Camera_t *cam,
...
int jpgutl_put_grey(u_char *dest_image, int image_size, u_char *input_image,
                    int width, int height, int quality, Camera_t *cam,
                    timespec *ts1, ctx_coord *box) {
