...
#include "web/webu_post.hpp"

/**************Callback functions for MHD **********************/

mhdrslt webup_iterate_post(void *ptr, enum MHD_ValueKind kind, const char *key,
                           const char *filename, const char *content_type,
                           const char *transfer_encoding, const char *data,
                           uint64_t off, size_t datasz) {
  (void)kind;
  (void)filename;
  (void)content_type;
...
void WebPostHandler::cam_delete() {
  int indx, maxcnt;
...
void WebPostHandler::parse_cmd() {
  ...
  for (indx = 0; indx < post_sz; indx++) {
    ...
    MOTPLS_LOG(DBG, TYPE_STREAM, NO_ERRNO, "key: %s  value: %s ",
               post_info[indx].key_nm, post_info[indx].key_val);
  }

  if (post_cmd == "") {
    MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO, "Invalid post request.  No command");
    return;
  }

  /* REST style urls: /api/cameras/{camid}/actions carry the camid in the
   * url instead of the post body.
   */
  if (webua->device_id == -1) {
    if ((webua->uri_cmd1 == "cameras") && (webua->uri_cmd3 == "actions")) {
      bool is_nbr = (webua->uri_cmd2.length() > 0);
      for (indx = 0; indx < (int)webua->uri_cmd2.length(); indx++) {
        if ((webua->uri_cmd2[(uint)indx] > '9') ||
            (webua->uri_cmd2[(uint)indx] < '0')) {
          is_nbr = false;
        }
      }
      if (is_nbr) {
        webua->device_id = atoi(webua->uri_cmd2.c_str());
      }
    }
  }
  ...
  if (webua->device_id != 0) {
    ...
    if (webua->camindx == -1) {
      MOTPLS_LOG(ERR, TYPE_ALL, NO_ERRNO,
                 "Invalid request.  Device id %d not found", webua->device_id);
      webua->device_id = -1;
      return;
    }
  }
...
void WebPostHandler::action_eventend() {
  int indx;
...
void WebPostHandler::action_eventstart() {
  int indx;
...
void WebPostHandler::action_snapshot() {
  int indx;
...
void WebPostHandler::action_pause_on() {
  int indx;
...
void WebPostHandler::action_pause_off() {
  int indx;
...
void WebPostHandler::action_pause_schedule() {
  int indx;
...
void WebPostHandler::action_restart() {
  int indx;
  ...
  if (webua->device_id == 0) {
    ...
    MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO, _("Restarting camera %d"),
...
void WebPostHandler::action_stop() {
  int indx;
  ...
  if (webua->device_id == 0) {
    for (indx = 0; indx < app->cam_cnt; indx++) {
      MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO, _("Stopping cam %d"),
...
void WebPostHandler::action_user() {
  int indx, indx2;
  Camera_t *cam;
  std::string tmp;
  ...
  if (webua->device_id == 0) {
    for (indx = 0; indx < app->cam_cnt; indx++) {
      ...
      MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO,
                 _("Executing user action on cam %d"), cam->cfg->device_id);
    ...
    MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO, _("Executing user action on cam %d"),
               cam->cfg->device_id);
    util_exec_command(cam, cam->cfg->on_action_user.c_str(), NULL);
  }
}
...
void WebPostHandler::write_config() {
  int indx;
...
void WebPostHandler::config_set(int indx_parm, std::string parm_vl) {
  ...
  if (webua->device_id == 0) {
    ...
    if (parm_ct == PARM_CAT_00) {
      ...
      for (indx = 0; indx < app->cam_cnt; indx++) {
        if (app->cam_list[indx]->handler_running == false) {
          ...
          app->cam_list[indx]->conf_src->edit_get(parm_nm, parm_vl_dev,
                                                  parm_ct);
      ...
      for (indx = 0; indx < app->snd_cnt; indx++) {
        app->snd_list[indx]->conf_src->edit_get(parm_nm, parm_vl_dev, parm_ct);
        if (parm_vl_dev == parm_vl_dflt) {
    ...
    if ((parm_ct == PARM_CAT_00) || (parm_ct == PARM_CAT_13) ||
        (parm_ct == PARM_CAT_15)) {
      return;
    }
    MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, "Config edit set. %s:%s",
               parm_nm.c_str(), parm_vl.c_str());
    app->cam_list[webua->camindx]->conf_src->edit_set(parm_nm, parm_vl);
    if (app->cam_list[webua->camindx]->handler_running == true) {
      config_restart_set("cam", webua->camindx);
    } else {
      app->cam_list[webua->camindx]->cfg->edit_set(parm_nm, parm_vl);
    }
  }
}

void WebPostHandler::config_restart_set(std::string p_type, int p_indx) {
  int indx;
...
void WebPostHandler::config_restart_reset() {
  ...
  for (indx = 0; indx < post_sz; indx++) {
    if (mystrne(post_info[indx].key_nm, "command") &&
      ...
      while (config_parms[indx2].parm_name != "") {
        if ((config_parms[indx2].webui_level >
             app->conf_src->webcontrol_parms) ||
  ...
  for (indx = 0; indx < restart_list.size(); indx++) {
    if (restart_list[indx].restart == true) {
      if (restart_list[indx].comp_type == "log") {
        motlog->restart = true;
        MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, "Restart request for log");
      } else if (restart_list[indx].comp_type == "webu") {
        app->webu->restart = true;
        MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, "Restart request for webcontrol");
      } else if (restart_list[indx].comp_type == "dbse") {
        app->dbse->restart = true;
        MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, "Restart request for database");
      } else if (restart_list[indx].comp_type == "cam") {
        app->cam_list[restart_list[indx].comp_indx]->restart = true;
        MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, "Restart request for camera %d",
        ...
        MOTPLS_LOG(DBG, TYPE_ALL, NO_ERRNO, "Restart request for sound %d",
                   app->cam_list[restart_list[indx].comp_indx]->cfg->device_id);
      } else {
...
void WebPostHandler::process_actions() {
  parse_cmd();
  ...
  } else if ((post_cmd == "pan_left") || (post_cmd == "pan_right") ||
             (post_cmd == "tilt_up") || (post_cmd == "tilt_down") ||
             (post_cmd == "zoom_in") || (post_cmd == "zoom_out")) {
    ptz();
...
void WebPostHandler::iterate_post_append(int indx, const char *data,
                                         size_t datasz) {
  post_info[indx].key_val = (char *)realloc(
      post_info[indx].key_val, post_info[indx].key_sz + datasz + 1);

  memset(post_info[indx].key_val + post_info[indx].key_sz, 0, datasz + 1);

  if (datasz > 0) {
    memcpy(post_info[indx].key_val + post_info[indx].key_sz, data, datasz);
  }
...
void WebPostHandler::iterate_post_new(const char *key, const char *data,
                                      size_t datasz) {
  int retcd;
  ...
  if (post_sz == 1) {
    ...
    post_info = (ctx_key *)realloc(post_info, (uint)post_sz * sizeof(ctx_key));
  }
...
mhdrslt WebPostHandler::iterate_post(const char *key, const char *data,
                                     size_t datasz) {
  int indx;
...
mhdrslt WebPostHandler::processor_init() {
  post_processor = MHD_create_post_processor(
      webua->connection, WEBUI_POST_BFRSZ, webup_iterate_post, (void *)this);
...
mhdrslt WebPostHandler::processor_start(const char *upload_data,
  ...
  if (*upload_data_size != 0) {
    ...
    *upload_data_size = 0;
    ...
    if (webua->uri_camid == "api") {
      /* REST api requests receive a JSON acknowledgement instead of the
       * full html page.
       */
      webua->resp_type = WEBUI_RESP_JSON;
      webua->resp_page = "{\"result\" : \"ok\",\"command\" : \"" + post_cmd +
                         "\"}";
      webua->mhd_send();
    } else {
      /* Legacy interface posts are processed; the Vue UI polls state
       * through the API so only a minimal acknowledgement is needed.
       */
      webua->resp_page =
          "<html><body>Action processed.</body></html>";
      webua->mhd_send();
    }
    retcd = MHD_YES;
  }
...
WebPostHandler::WebPostHandler(WebAnswer *p_webua) {
  app = p_webua->app;
  webu = p_webua->webu;
  webua = p_webua;
