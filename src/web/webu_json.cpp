...
std::string WebJson::escstr(std::string invar) {
  std::string outvar;
  size_t indx;
  for (indx = 0; indx < invar.length(); indx++) {
    if (invar[indx] == '\\' || invar[indx] == '\"') {
      outvar += '\\';
    }
    outvar += invar[indx];
  }
...
void WebJson::parms_item(Config *conf, int indx_parm) {
  ...
  conf->edit_get(config_parms[indx_parm].parm_name, parm_orig,
  ...
  if (config_parms[indx_parm].parm_type == PARM_TYP_INT) {
    webua->resp_page +=
        ...
        " \"value\":" + parm_val + ",\"enabled\":" + parm_enable +
    ...
    if (parm_val == "on") {
      webua->resp_page +=
          "\"" + config_parms[indx_parm].parm_name + "\"" + ":{" +
          " \"value\":true" + ",\"enabled\":" + parm_enable +
          ",\"category\":" + std::to_string(config_parms[indx_parm].parm_cat) +
          ",\"type\":\"" + conf->type_desc(config_parms[indx_parm].parm_type) +
          "\"" + "}";
    } else {
      webua->resp_page +=
          "\"" + config_parms[indx_parm].parm_name + "\"" + ":{" +
          " \"value\":false" + ",\"enabled\":" + parm_enable +
          ",\"category\":" + std::to_string(config_parms[indx_parm].parm_cat) +
          ",\"type\":\"" + conf->type_desc(config_parms[indx_parm].parm_type) +
          "\"" + "}";
    }
  } else if (config_parms[indx_parm].parm_type == PARM_TYP_LIST) {
    conf->edit_list(config_parms[indx_parm].parm_name, parm_list,
                    config_parms[indx_parm].parm_cat);
...
void WebJson::parms_one(Config *conf) {
  int indx_parm;
  bool first;
  std::string response;
...
void WebJson::parms_all() {
  ...
  for (indx_cam = 0; indx_cam < app->cam_cnt; indx_cam++) {
    webua->resp_page +=
        ",\"cam" + std::to_string(app->cam_list[indx_cam]->cfg->device_id) +
        "\": ";
...
void WebJson::cameras_list() {
  ...
  for (indx_cam = 0; indx_cam < app->cam_cnt; indx_cam++) {
    ...
    if (cam->cfg->device_name == "") {
      ...
      webua->resp_page +=
          "{\"name\": \"" + escstr(cam->cfg->device_name) + "\"";
    }
    webua->resp_page += ",\"id\": " + strid;
    webua->resp_page +=
        ",\"all_xpct_st\": " + std::to_string(cam->all_loc.xpct_st);
    webua->resp_page +=
        ",\"all_xpct_en\": " + std::to_string(cam->all_loc.xpct_en);
    webua->resp_page +=
        ",\"all_ypct_st\": " + std::to_string(cam->all_loc.ypct_st);
    webua->resp_page +=
        ",\"all_ypct_en\": " + std::to_string(cam->all_loc.ypct_en);
    webua->resp_page +=
        ",\"url\": \"" + webua->hostfull + "/" + strid + "/\"} ";
  }
  webua->resp_page += "}";
}

void WebJson::categories_list() {
  int indx_cat;
  ...
  while (indx_cat != PARM_CAT_MAX) {
    ...
    webua->resp_page +=
        "{\"name\":\"" + catnm_short + "\",\"display\":\"" + catnm_long + "\"}";
  ...
  for (indx2 = 0; indx2 < flst.size(); indx2++) {
    if (flst[indx2].found == true) {
      if ((flst[indx2].file_sz / 1000) < 1000) {
        snprintf(fmt, PATH_MAX, "%.1fKB", ((double)flst[indx2].file_sz / 1000));
...
void WebJson::movies() {
  ...
  if (webua->cam == NULL) {
    ...
    indx_req = -1;
    for (indx_cam = 0; indx_cam < app->cam_cnt; indx_cam++) {
      if (webua->cam->cfg->device_id ==
          app->cam_list[indx_cam]->cfg->device_id) {
        indx_req = indx_cam;
      }
    }
...
void WebJson::status_vars(int indx_cam) {
  char buf[32];
  ...
  if (cam->connectionlosttime.tv_sec != 0) {
    ...
    strftime(buf, sizeof(buf), "%FT%T", &timestamp_tm);
    webua->resp_page +=
        ",\"connection_lost_time\":\"" + std::string(buf) + "\"";
  } else {
  ...
  for (indx = 0; indx < motlog->log_vec.size(); indx++) {
    ...
    if ((motlog->log_vec[indx].log_nbr > mtoi(webua->uri_cmd2)) &&
      ...
      webua->resp_page += "\"logmsg\" :\"" +
                          escstr(motlog->log_vec[indx].log_msg.substr(
                              0, motlog->log_vec[indx].log_msg.length() - 1)) +
                          "\" ";
      webua->resp_page += "}";
      cnt++;
    }
...
WebJson::WebJson(WebAnswer *p_webua) {
  app = p_webua->app;
  webu = p_webua->webu;
  webua = p_webua;
}
