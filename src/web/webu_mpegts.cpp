...
static int webu_mpegts_avio_buf(void *opaque, myuint *buf, int buf_size) {
  ...
  webu_mpegts = (WebMpegTs *)opaque;
  return webu_mpegts->avio_buf(buf, buf_size);
}

static ssize_t webu_mpegts_response(void *cls, uint64_t pos, char *buf,
                                    size_t max) {
  WebMpegTs *webu_mpegts;
  (void)pos;
  webu_mpegts = (WebMpegTs *)cls;
  return webu_mpegts->response(buf, max);
}

/********Class Functions ****************************************************/

int WebMpegTs::pic_send(unsigned char *img) {
  int retcd;
  char errstr[128];
  ...
  picture->data[0] = img;
  picture->data[1] = picture->data[0] + (ctx_codec->width * ctx_codec->height);
  picture->data[2] =
...
void WebMpegTs::resetpos() {
  stream_pos = 0;
  webus->resp_used = 0;
}

int WebMpegTs::getimg() {
...
int WebMpegTs::avio_buf(myuint *buf, int buf_size) {
  if (webus->resp_size < (size_t)buf_size + webus->resp_used) {
    webus->resp_size = (size_t)buf_size + webus->resp_used;
    webus->resp_image =
        (unsigned char *)realloc(webus->resp_image, webus->resp_size);
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("resp_image reallocated %d %d %d"),
               webus->resp_size, webus->resp_used, buf_size);
  }
...
ssize_t WebMpegTs::response(char *buf, size_t max) {
  size_t sent_bytes;
...
int WebMpegTs::open_mpegts() {
  ...
  fmtctx->pb = avio_alloc_context(buf_image, (int)aviobuf_sz, 1, this, NULL,
                                  &webu_mpegts_avio_buf, NULL);
  fmtctx->flags = AVFMT_FLAG_CUSTOM_IO;

  retcd = avformat_write_header(fmtctx, &opts);
  if (retcd < 0) {
    av_strerror(retcd, errstr, sizeof(errstr));
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("Failed to write header!: %s"),
               errstr);
    av_dict_free(&opts);
    return -1;
  }
...
WebMpegTs::WebMpegTs(WebAnswer *p_webua, WebStream *p_webus) {
  app = p_webua->app;
  webu = p_webua->webu;
  webua = p_webua;
  webus = p_webus;
