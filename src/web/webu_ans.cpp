...
static mhdrslt webua_connection_values(void *cls, enum MHD_ValueKind kind,
                                       const char *src_key,
                                       const char *src_val) {
  (void)kind;
  std::string parm_val;
...
int WebAnswer::parseurl() {
  ...
  if (url.substr(0, baselen) != app->cfg->webcontrol_base_path) {
    return -1;
  }
...
void WebAnswer::parms_edit() {
  ...
  MOTPLS_LOG(DBG, TYPE_STREAM, NO_ERRNO,
             ...
             uri_camid.c_str(), camindx, uri_cmd1.c_str(), uri_cmd2.c_str(),
             uri_cmd3.c_str());
}

/* Log the ip of the client connecting*/
void WebAnswer::clientip_get() {
  ...
  if (is_ipv6) {
    ...
    ip_dst =
        inet_ntop(AF_INET6, &con_socket6->sin6_addr, client, WEBUI_LEN_URLI);
    if (ip_dst == NULL) {
      clientip = "Unknown";
    } else {
...
/* Get the hostname */
void WebAnswer::hostname_get() {
  const char *hdr;

  hdr = MHD_lookup_connection_value(connection, MHD_HEADER_KIND,
                                    MHD_HTTP_HEADER_HOST);
  if (hdr == NULL) {
    hostfull = "//localhost:" + std::to_string(app->cfg->webcontrol_port) +
               app->cfg->webcontrol_base_path;
  } else {
    hostfull = "//" + std::string(hdr) + app->cfg->webcontrol_base_path;
  }
...
void WebAnswer::failauth_log(bool userid_fail) {
  timespec tm_cnct;
  ctx_webu_clients clients;
  std::list<ctx_webu_clients>::iterator it;

  MOTPLS_LOG(ALR, TYPE_STREAM, NO_ERRNO, _("Failed authentication from %s"),
             clientip.c_str());
  ...
  while (it != webu->wb_clients.end()) {
    if (it->clientip == clientip) {
      if (it->authenticated == false) {
        MOTPLS_LOG(INF, TYPE_ALL, NO_ERRNO, _("Connection from: %s"),
                   clientip.c_str());
      }
...
mhdrslt WebAnswer::failauth_check() {
  timespec tm_cnct;
  std::list<ctx_webu_clients>::iterator it;
  std::string tmp;
  ...
  while (it != webu->wb_clients.end()) {
    if ((it->clientip == clientip) &&
      ...
      MOTPLS_LOG(EMG, TYPE_STREAM, NO_ERRNO, "Ignoring connection from: %s",
                 clientip.c_str());
...
mhdrslt WebAnswer::mhd_digest_fail(int signal_stale) {
  ...
  response = MHD_create_response_from_buffer(
      resp_page.length(), (void *)resp_page.c_str(), MHD_RESPMEM_PERSISTENT);

  if (response == NULL) {
    return MHD_NO;
  }

  retcd = MHD_queue_auth_fail_response(
      connection, auth_realm, auth_opaque, response,
...
mhdrslt WebAnswer::mhd_digest() {
  ...
  retcd =
      MHD_digest_auth_check(connection, auth_realm, auth_user, auth_pass, 300);
...
/* Create a authorization denied response to user*/
mhdrslt WebAnswer::mhd_basic_fail() {
  struct MHD_Response *response;
  int retcd;
  ...
  if (app->cfg->webcontrol_authentication == "") {
    ...
    if (app->cfg->webcontrol_auth_method != "none") {
      MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO,
                 _("No webcontrol user:pass provided"));
    }
    return MHD_YES;
  }
...
void WebAnswer::gzip_deflate() {
  ...
  */
  ...
  if (retcd < Z_OK) {
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("deflate failed: %d"), retcd);
    gzip_size = 0;
  } else {
  ...
  retcd = deflateEnd(&zs);
  if (retcd < Z_OK) {
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("deflateEnd failed: %d"), retcd);
    gzip_size = 0;
  }

  if (zs.avail_in != 0) {
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("deflate failed avail in: %d"),
               zs.avail_in);
    gzip_size = 0;
  }
}
...
void WebAnswer::mhd_send() {
  ...
  if (gzip_encode == true) {
    ...
    if (gzip_size == 0) {
      ...
      response = MHD_create_response_from_buffer(gzip_size, (void *)gzip_resp,
                                                 MHD_RESPMEM_PERSISTENT);
    }
  } else {
    response = MHD_create_response_from_buffer(
  ...
  if (webu->wb_headers->params_cnt > 0) {
    for (indx = 0; indx < webu->wb_headers->params_cnt; indx++) {
      MHD_add_response_header(
          response, webu->wb_headers->params_array[indx].param_name.c_str(),
...
void WebAnswer::bad_request() {
  resp_page = "<!DOCTYPE html>\n"
              "<html>\n"
              "<body>\n"
              "<p>Bad Request</p>\n"
...
void WebAnswer::answer_get() {
  MOTPLS_LOG(DBG, TYPE_STREAM, NO_ERRNO, "processing get: %s",
             uri_cmd1.c_str());

  if (uri_camid == "api") {
    if (webu_api == nullptr) {
      webu_api = new WebApi(this);
    }
    webu_api->main();

  } else if ((uri_cmd1 == "mjpg") || (uri_cmd1 == "mpegts") ||
             (uri_cmd1 == "static")) {
    if (webu_stream == nullptr) {
      webu_stream = new WebStream(this);
    }
    gzip_encode = false;
    webu_stream->main();
    ...
    if (webu_json == nullptr) {
      webu_json = new WebJson(this);
    }
    webu_json->main();

  } else {
    if (webu_static == nullptr) {
      webu_static = new WebStatic(this);
    }
    webu_static->main();
  }
}
...
mhdrslt WebAnswer::answer_main(struct MHD_Connection *p_connection,
  ...
  MHD_get_connection_values(p_connection, MHD_HEADER_KIND,
                            webua_connection_values, this);
  ...
  if (mhd_first) {
    ...
    if (mystreq(method, "POST")) {
      if (webu_post == nullptr) {
        webu_post = new WebPostHandler(this);
      }
...
void WebAnswer::deinit_counter() {
  ...
  for (indx = cam_min; indx < cam_max; indx++) {
    ...
    if ((cnct_type > WEBUI_CNCT_JPG_MIN) && (cnct_type < WEBUI_CNCT_JPG_MAX)) {
    ...
    if ((strm->all_cnct == 0) && (strm->jpg_cnct == 0) &&
        (strm->ts_cnct == 0) && (p_cam->passflag)) {
...
WebAnswer::WebAnswer(NvrApp *p_app, const char *uri) {
  app = p_app;
  webu = p_app->webu;
