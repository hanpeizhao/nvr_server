...
static ssize_t webu_mjpeg_response(void *cls, uint64_t pos, char *buf,
                                   size_t max) {
  WebStream *webu_stream = (WebStream *)cls;
  (void)pos;
  return webu_stream->mjpeg_response(buf, max);
}

void WebStream::set_fps() {
...
bool WebStream::check_finish() {
  if (webu->finish) {
    resp_used = 0;
    return true;
  }
  if (webua->cam != NULL) {
    if ((webua->cam->finish == true) || (webua->cam->passflag == false)) {
      resp_used = 0;
      return true;
    }
  }
...
bool WebStream::all_ready() {
  int indx, indx1;
  Camera_t *p_cam;

  for (indx = 0; indx < app->cam_cnt; indx++) {
    p_cam = app->cam_list[indx];
    if ((p_cam->device_status == STATUS_OPENED) && (p_cam->passflag == false)) {
      indx1 = 0;
      while (indx1 < 1000) {
        SLEEP(0, 1000);
        if (p_cam->passflag) {
      ...
      if (p_cam->passflag == false) {
        MOTPLS_LOG(DBG, TYPE_STREAM, NO_ERRNO, "Camera %d not ready",
                   p_cam->cfg->device_id);
        return false;
      }
    }
...
void WebStream::mjpeg_all_img() {
  ...
  memcpy(resp_image + header_len, strm->jpg_data, (uint)strm->jpg_sz);
...
ssize_t WebStream::mjpeg_response(char *buf, size_t max) {
  size_t sent_bytes;
...
void WebStream::static_all_img() {
  ...
  memcpy(resp_image, strm->jpg_data, (uint)strm->jpg_sz);
...
bool WebStream::valid_request() {
  ...
  if (webua->device_id < 0) {
    MOTPLS_LOG(ERR, TYPE_STREAM, NO_ERRNO, _("Invalid camera specified: %s"),
               webua->url.c_str());
...
mhdrslt WebStream::stream_mjpeg() {
  mhdrslt retcd;
  ...
  response = MHD_create_response_from_callback(
      MHD_SIZE_UNKNOWN, 1024, &webu_mjpeg_response, (void *)this, NULL);
  if (response == NULL) {
...
mhdrslt WebStream::stream_static() {
  mhdrslt retcd;
  struct MHD_Response *response;
  char resp_head[20];
  int indx;
  ...
  response = MHD_create_response_from_buffer(resp_size, (void *)resp_image,
...
mhdrslt WebStream::main() {
  ...
  if (webua->uri_cmd1 == "static") {
    ...
    if (webu_mpegts == nullptr) {
      webu_mpegts = new WebMpegTs(webua, this);
    }
...
WebStream::WebStream(WebAnswer *p_webua) {
  app = p_webua->app;
  webu = p_webua->webu;
  webua = p_webua;
  webu_mpegts = nullptr;
