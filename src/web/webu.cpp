...
#include "web/webu_mpegts.hpp"
#include "media/cam/video_v4l2.hpp"

/* Initialize the MHD answer */
static void *webu_mhd_init(void *cls, const char *uri,
                           struct MHD_Connection *connection) {
  (void)connection;
  NvrApp *p_app = (NvrApp *)cls;
  WebAnswer *webua;

  mythreadname_set("wc", 0, NULL);

  webua = new WebAnswer(p_app, uri);
...
static void webu_mhd_deinit(void *cls, struct MHD_Connection *connection,
  ...
  (void)cls;
  (void)toe;
  WebAnswer *webua = (WebAnswer *)*con_cls;
...
static mhdrslt mhd_answer(void *cls, struct MHD_Connection *connection,
...
void Web::mhd_features_digest() {
  ...
  if (retcd == MHD_YES) {
    MOTPLS_LOG(DBG, TYPE_STREAM, NO_ERRNO,
               _("Digest authentication: available"));
  } else {
    if (app->cfg->webcontrol_auth_method == "digest") {
      MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO,
                 _("Digest authentication: disabled"));
      app->cfg->webcontrol_auth_method = "none";
    } else {
      MOTPLS_LOG(INF, TYPE_STREAM, NO_ERRNO,
                 _("Digest authentication: disabled"));
...
void Web::mhd_features_ipv6() {
#if MHD_VERSION < 0x00094400
  if (mhdst->ipv6) {
    MOTPLS_LOG(INF, TYPE_STREAM, NO_ERRNO,
               _("libmicrohttpd libary too old ipv6 disabled"));
    if (mhdst->ipv6) {
      mhdst->ipv6 = 0;
    }
  }
...
void Web::mhd_features_tls() {
#if MHD_VERSION < 0x00094400
  if (mhdst->tls_use) {
    MOTPLS_LOG(INF, TYPE_STREAM, NO_ERRNO,
               _("libmicrohttpd libary too old SSL/TLS disabled"));
    mhdst->tls_use = false;
  }
#else
  mhdrslt retcd;
...
void Web::mhd_loadfile(std::string fname, std::string &filestr) {
...
void Web::mhd_checktls() {
  if (mhdst->tls_use) {
    if ((app->cfg->webcontrol_cert == "") || (mhdst->tls_cert == "")) {
      MOTPLS_LOG(
          NTC, TYPE_STREAM, NO_ERRNO,
...
void Web::mhd_opts_localhost() {
  if (app->cfg->webcontrol_localhost) {
    if (mhdst->ipv6) {
      ...
      mhdst->mhd_ops[mhdst->mhd_opt_nbr].ptr_value =
          (struct sosockaddr *)(&mhdst->lpbk_ipv6);
      mhdst->mhd_opt_nbr++;
      ...
      mhdst->mhd_ops[mhdst->mhd_opt_nbr].value = 0;
      mhdst->mhd_ops[mhdst->mhd_opt_nbr].ptr_value =
          (struct sockaddr *)(&mhdst->lpbk_ipv4);
      mhdst->mhd_opt_nbr++;
    }
  }
}
...
void Web::init_actions() {
  std::string parm_vl;

  wb_actions = new ctx_params;
  util_parms_parse(wb_actions, "webcontrol_actions",
                   app->cfg->webcontrol_actions);
...
void Web::start_daemon_port1() {
  ...
  mhdst->mhd_ops = (struct MHD_OptionItem *)mymalloc(
      sizeof(struct MHD_OptionItem) * WEBUI_MHD_OPTS);
  mhd_features();
  mhd_opts();
  mhd_flags();

  wb_daemon = MHD_start_daemon(
      mhdst->mhd_flags, (uint16_t)app->cfg->webcontrol_port, NULL, NULL,
      &mhd_answer, app, MHD_OPTION_ARRAY, mhdst->mhd_ops, MHD_OPTION_END);
  ...
  if (wb_daemon == nullptr) {
    ...
    MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO, _("Started webcontrol on port %d"),
               app->cfg->webcontrol_port);
  }
  delete mhdst;
  mhdst = nullptr;
...
void Web::start_daemon_port2() {
  ...
  wb_daemon2 = MHD_start_daemon(
      mhdst->mhd_flags, (uint16_t)app->cfg->webcontrol_port2, NULL, NULL,
      &mhd_answer, app, MHD_OPTION_ARRAY, mhdst->mhd_ops, MHD_OPTION_END);
...
void Web::startup() {
  ...
  MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO, _("Starting webcontrol on port %d"),
             app->cfg->webcontrol_port);

  wb_headers = new ctx_params;
  util_parms_parse(wb_headers, "webcontrol_headers",
                   app->cfg->webcontrol_headers);
...
void Web::shutdown() {
  ...
  if (chkcnt >= 1000) {
    MOTPLS_LOG(NTC, TYPE_STREAM, NO_ERRNO,
               _("Excessive wait closing webcontrol"));
  }
