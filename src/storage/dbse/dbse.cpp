...
static void *dbse_handler(void *arg) {
  ((DataBase_t *)arg)->handler();
  return nullptr;
}

#ifdef HAVE_DBSE

void DataBase_t::cols_vec_add(std::string nm, std::string typ) {
...
void DataBase_t::cols_vec_create() {
  col_names.clear();
...
void DataBase_t::item_assign(std::string col_nm, std::string col_val) {
  struct stat statbuf;
...
void DataBase_t::sql_motpls(std::string &sql) {
  std::string delimit;
...
void DataBase_t::sql_motpls(std::string &sql, std::string col_p1,
                            std::string col_p2) {
  if ((is_open == false) || (finish == true)) {
    return;
  }
  ...
  if ((dbse_action == DBSE_COLS_ADD) && (col_p1 != "") && (col_p2 != "")) {
    sql = "Alter table ztnvr add column ";
    sql += col_p1 + " " + col_p2 + " ;";
  } else if ((dbse_action == DBSE_COLS_RENAME) && (col_p1 != "") &&
             (col_p2 != "")) {
    sql = "Alter table ztnvr rename column ";
...
static int dbse_sqlite3db_cb(void *ptr, int arg_nb, char **arg_val,
                             char **col_nm) {
  DataBase_t *dbse = (DataBase_t *)ptr;
  dbse->sqlite3db_cb(arg_nb, arg_val, col_nm);
  return 0;
}

void DataBase_t::sqlite3db_exec(std::string sql) {
  int retcd;
  char *errmsg = nullptr;

  if ((finish == true) || (database_sqlite3db == nullptr) ||
      (is_open == false)) {
    return;
  }

  MOTPLS_LOG(DBG, TYPE_DB, NO_ERRNO, "Executing query");
  retcd = sqlite3_exec(database_sqlite3db, sql.c_str(), nullptr, 0, &errmsg);
  if (retcd != SQLITE_OK) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("SQLite error was %s"), errmsg);
    sqlite3_free(errmsg);
  }
  MOTPLS_LOG(DBG, TYPE_DB, NO_ERRNO, "Finished query");
}

void DataBase_t::sqlite3db_cb(int arg_nb, char **arg_val, char **col_nm) {
  int indx, indx2;
  ...
  if (dbse_action == DBSE_TBL_CHECK) {
    for (indx = 0; indx < arg_nb; indx++) {
      if (mystrceq(arg_val[indx], "ztnvr")) {
        table_ok = true;
      }
    }
...
void DataBase_t::sqlite3db_cols_verify() {
  int retcd, indx;
  char *errmsg = 0;
  std::string sql;
  ...
  sql_motpls(sql);
  retcd = sqlite3_exec(database_sqlite3db, sql.c_str(), dbse_sqlite3db_cb, this,
                       &errmsg);
  if (retcd != SQLITE_OK) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Error retrieving table columns: %s"),
               errmsg);
    sqlite3_free(errmsg);
    return;
  }
  ...
  for (indx = 0; indx < col_names.size(); indx++) {
    if (col_names[indx].found == false) {
      ...
      sql_motpls(sql, col_names[indx].col_nm, col_names[indx].col_typ);
      sqlite3db_exec(sql.c_str());
    }
  }
}

void DataBase_t::sqlite3db_cols_rename() {
  int retcd, indx;
  char *errmsg = 0;
  std::string sql, tmp;
...
void DataBase_t::sqlite3db_init() {
  int retcd;
  ...
  MOTPLS_LOG(NTC, TYPE_DB, NO_ERRNO, _("SQLite3 Database filename %s"),
             app->cfg->database_dbname.c_str());
  retcd = sqlite3_open(app->cfg->database_dbname.c_str(), &database_sqlite3db);
  if (retcd != SQLITE_OK) {
    err_open = sqlite3_errmsg(database_sqlite3db);
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Can't open database %s : %s"),
               app->cfg->database_dbname.c_str(), err_open);
    sqlite3_close(database_sqlite3db);
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Could not initialize database %s"),
               app->cfg->database_dbname.c_str());
    is_open = false;
    database_sqlite3db = nullptr;
    return;
  ...
  is_open = true;
  MOTPLS_LOG(NTC, TYPE_DB, NO_ERRNO, _("database_busy_timeout %d msec"),
             app->cfg->database_busy_timeout);
  retcd =
      sqlite3_busy_timeout(database_sqlite3db, app->cfg->database_busy_timeout);
  if (retcd != SQLITE_OK) {
    err_open = sqlite3_errmsg(database_sqlite3db);
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("database_busy_timeout failed %s"),
               err_open);
  }
  ...
  retcd = sqlite3_exec(database_sqlite3db, sql.c_str(), dbse_sqlite3db_cb, this,
                       &err_qry);
  if (retcd != SQLITE_OK) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Error checking table: %s"), err_qry);
  ...
  if (table_ok == false) {
    ...
    if (retcd != SQLITE_OK) {
      MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Error creating table: %s"),
                 err_qry);
      sqlite3_free(err_qry);
      return;
    }
...
void DataBase_t::sqlite3db_filelist(std::string sql) {
  int retcd;
  char *errmsg = nullptr;
  ...
  retcd = sqlite3_exec(database_sqlite3db, sql.c_str(), dbse_sqlite3db_cb, this,
                       &errmsg);
  if (retcd != SQLITE_OK) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Error retrieving table: %s"), errmsg);
    sqlite3_free(errmsg);
    return;
  }
}

void DataBase_t::sqlite3db_close() {
  if (app->cfg->database_type == "sqlite3") {
...
void DataBase_t::mariadb_exec(std::string sql) {
  int retcd;
  ...
  if (retcd != 0) {
    ...
    MOTPLS_LOG(ERR, TYPE_DB, SHOW_ERRNO,
               _("MariaDB query '%s' failed. %s error code %d"), sql.c_str(),
...
void DataBase_t::mariadb_recs(std::string sql) {
  ...
  retcd = mysql_query(database_mariadb, sql.c_str());
  if (retcd != 0) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Query error: %s"), sql.c_str());
    shutdown();
    return;
  }
  ...
  if (qry_result == nullptr) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Query store error: %s"), sql.c_str());
    shutdown();
    return;
  }
    ...
    while (qry_row != nullptr) {
      for (indx = 0; indx < qry_fields; indx++) {
        if (qry_row[indx] != nullptr) {
          if (mystrceq(qry_row[indx], "ztnvr")) {
            table_ok = true;
          }
        }
      }
...
void DataBase_t::mariadb_cols_verify() {
  std::string sql;
  int indx;
...
void DataBase_t::mariadb_cols_rename() {
  std::string sql, tmp;
  int indx;
...
void DataBase_t::mariadb_setup() {
  ...
  if (table_ok == false) {
    MOTPLS_LOG(INF, TYPE_DB, NO_ERRNO, _("Creating ztnvr table"));
    dbse_action = DBSE_TBL_CREATE;
    sql_motpls(sql);
    mariadb_exec(sql.c_str());
  }
...
void DataBase_t::mariadb_init() {
  ...
  if (mysql_real_connect(
          database_mariadb, app->cfg->database_host.c_str(),
          app->cfg->database_user.c_str(), app->cfg->database_password.c_str(),
          app->cfg->database_dbname.c_str(), (uint)app->cfg->database_port,
          nullptr, 0) == nullptr) {

    MOTPLS_LOG(
        ERR, TYPE_DB, NO_ERRNO,
        _("Cannot connect to MariaDB database %s on host %s with user %s"),
        app->cfg->database_dbname.c_str(), app->cfg->database_host.c_str(),
        app->cfg->database_user.c_str());
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("MariaDB error was %s"),
               mysql_error(database_mariadb));
    shutdown();
    return;
  }
  is_open = true;
  mysql_options(database_mariadb, MYSQL_OPT_RECONNECT, &my_true);
...
void DataBase_t::mariadb_filelist(std::string sql) {
  dbse_action = DBSE_MOV_SELECT;
  mariadb_recs(sql.c_str());
}
...
void DataBase_t::pgsqldb_exec(std::string sql) {
  PGresult *res;
  ...
  if (PQstatus(database_pgsqldb) == CONNECTION_BAD) {
    ...
    MOTPLS_LOG(ERR, TYPE_DB, SHOW_ERRNO, "PGSQL query failed: [%s]  %s %s",
               sql.c_str(), PQresStatus(PQresultStatus(res)),
               PQresultErrorMessage(res));
  }
  PQclear(res);
}

void DataBase_t::pgsqldb_close() {
...
void DataBase_t::pgsqldb_recs(std::string sql) {
  PGresult *res;
  ...
  if (dbse_action == DBSE_TBL_CHECK) {
    ...
    for (indx = 0; indx < rows; indx++) {
      for (indx2 = 0; indx2 < cols; indx2++) {
        if (mystrceq("table_nm", PQfname(res, indx2)) &&
            mystrceq("ztnvr", PQgetvalue(res, indx, indx2))) {
          table_ok = true;
        }
      }
    }
    ...
    for (indx = 0; indx < cols; indx++) {
      for (indx2 = 0; indx2 < col_names.size(); indx2++) {
        if (mystrceq(PQfname(res, indx), col_names[indx2].col_nm.c_str())) {
          col_names[indx2].found = true;
        }
      }
    }
...
void DataBase_t::pgsqldb_cols_verify() {
  std::string sql;
  int indx;
...
void DataBase_t::pgsqldb_cols_rename() {
  int indx;
  std::string sql, tmp;
...
void DataBase_t::pgsqldb_filelist(std::string sql) {
  dbse_action = DBSE_MOV_SELECT;
  pgsqldb_recs(sql.c_str());
}
...
void DataBase_t::filelist_get(std::string sql, vec_files &p_flst) {
  int indx;
  if (dbse_open() == false) {
    p_flst.clear();
    return;
...
void DataBase_t::exec_sql(std::string sql) {
  if (dbse_open() == false) {
    return;
  }
...
void DataBase_t::exec(Camera_t *cam, std::string fname, std::string cmd) {
  std::string sql;
  ...
  MOTPLS_LOG(DBG, TYPE_DB, NO_ERRNO, "%s query: %s", cmd.c_str(), sql.c_str());

  exec_sql(sql);
}

void DataBase_t::filelist_add(Camera_t *cam, timespec *ts1, std::string ftyp,
                              std::string filenm, std::string fullnm,
                              std::string dirnm) {
  std::string sqlquery;
  struct stat statbuf;
  int64_t bsz;
...
void DataBase_t::dbse_edits() {
  int retcd = 0;

  if ((app->cfg->database_type != "") && (app->cfg->database_dbname == "")) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Invalid database name"));
    retcd = -1;
  }
  ...
  if ((app->cfg->database_type != "mariadb") &&
    ...
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Invalid database_type %s"),
               app->cfg->database_type.c_str());
    retcd = -1;
  }
  ...
  if ((app->cfg->database_type != "") && (retcd == -1)) {
    MOTPLS_LOG(ERR, TYPE_DB, NO_ERRNO, _("Database functionality disabled."));
    app->cfg->database_type = "";
  }
}

void DataBase_t::dbse_clean() {
...
void DataBase_t::startup() {
  is_open = false;
  dbse_edits();
  dbse_open();
}

bool DataBase_t::check_exit() {
...
void DataBase_t::handler_startup() {
  int retcd;
  ...
  if (handler_running == false) {
    ...
    handler_stop = false;
    restart = false;
    retcd = core::threadmgr::spawn_detached(handler_thread, &dbse_handler, this);
    if (retcd != 0) {
      MOTPLS_LOG(WRN, TYPE_ALL, NO_ERRNO,
                 _("Unable to start database handler thread."));
      handler_running = false;
      handler_stop = true;
    }
  }
...
void DataBase_t::handler_shutdown() {
  int waitcnt;
...
DataBase_t::DataBase_t(NvrApp *p_app) {
  app = p_app;
