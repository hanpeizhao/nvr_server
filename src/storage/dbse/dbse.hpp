...
/* Column item attributes in the ztnvr table */
struct ctx_col_item {
...
class DataBase_t {
public:
  DataBase_t(NvrApp *p_app);
  ~DataBase_t();
  void sqlite3db_cb(int arg_nb, char **arg_val, char **col_nm);
  pthread_mutex_t mutex_dbse;
  void exec(Camera_t *cam, std::string filename, std::string cmd);
  void exec_sql(std::string sql);
  void filelist_add(Camera_t *cam, timespec *ts1, std::string ftyp,
